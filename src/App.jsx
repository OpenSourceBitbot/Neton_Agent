...
const PAGES = {
  tools: { label: "nav.tools", icon: IconShield, page: ToolsPage },
...
export default function App() {
  ...
  // 安卓版扫码下载（NETON-mobile 仓库 Releases）：打开「关于」时懒加载二维码 SVG（后端离线渲染）
  const [androidQr, setAndroidQr] = useState("");
  const ANDROID_URL = "https://github.com/yxpil/NETON-mobile/releases/latest";
  useEffect(() => {
  ...
  const [chatSidebar, setChatSidebar] = useState(() => localStorage.getItem("NETON.chatSidebar") !== "0");
  const toggleChatSidebar = () =>
    setChatSidebar((v) => {
      localStorage.setItem("NETON.chatSidebar", v ? "0" : "1");
      return !v;
    });
  useEffect(() => {
    refresh();
  ...
  const NewChatBtn = () => (
    <button
      onClick={() => {
        setTab(PRIMARY);
        window.dispatchEvent(new CustomEvent("NETON-new-session"));
      }}
    ...
    <div className="flex h-screen flex-col overflow-hidden text-neutral-900 dark:text-neutral-100">
      ...
      <div className="flex min-h-0 flex-1">
        ...
        <aside className="flex w-12 shrink-0 flex-col items-center gap-1 px-1.5 py-2">
          ...
          <nav className="flex w-full flex-1 flex-col gap-0.5 pt-1">
            ...
            {SECONDARY.map((k) => (
              <div key={k} className="flex w-full flex-col gap-0.5">
                <NavItem k={k} />
                {k === "chat" && tab === "chat" && (
                  <>
                    <NewChatBtn />
                    ...
                    {chatSidebar && (
                      <RailMiniBtn
                        onClick={() => window.dispatchEvent(new CustomEvent("NETON-toggle-multi"))}
                        title={t("chat.multi")}
                      >
                        <IconCheck size={18} />
                      </RailMiniBtn>
        ...
        <main className="min-h-0 flex-1 overflow-hidden">
          {Object.entries(PAGES).map(([k, { page: Page }]) => (
            <div
              key={k}
              className={`h-full ${k === "chat" ? "overflow-hidden px-2 pb-3 pt-2" : "overflow-auto p-6"}`}
      ...
      {showAbout && (
        ...
        >
          ...
          >
            <img src={logoUrl} alt="NETON" className="mx-auto mb-2 h-14 w-14 rounded-full" />
            <div className="flex items-center justify-center gap-2">
              <h2 className="text-lg font-bold leading-tight">NETON</h2>
              {appVersion && (
            ...
            <p className="mx-auto mt-1 max-w-[330px] text-[10px] leading-relaxed text-neutral-400">
              {t("app.aboutSub")}
            </p>
            ...
            <div className="mt-3 flex flex-wrap items-center justify-center gap-2">
              ...
              <button
                onClick={() => invoke("open_external", { url: "https://github.com/yxpil/NETON" }).catch(() => {})}
                className="pill pill-outline pill-hover px-3 py-1 text-xs"
              >
                GitHub
              </button>
