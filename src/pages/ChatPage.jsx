...
export default function ChatPage({ onStats, visible, sidebarOpen, onToggleSidebar }) {
  ...
  const [ctxLimitK, setCtxLimitK] = useState(() => {
    const v = parseInt(localStorage.getItem("NETON.ctxLimitK"));
  ...
  const saveLimit = () => {
    ...
    if (Number.isFinite(v) && v >= 4 && v <= 2000 && v !== ctxLimitK) {
      setCtxLimitK(v);
      localStorage.setItem("NETON.ctxLimitK", String(v));
    }
  };
  ...
  useEffect(() => {
    loadSessions();
    // NETON 命令行等外部进程创建的会话也要出现：定期重拉列表（后端按 mtime 合并磁盘变更，代价极低）
  ...
  useEffect(() => {
    const h = () => newSession();
    window.addEventListener("NETON-new-session", h);
    return () => window.removeEventListener("NETON-new-session", h);
  }, []);

  // 图标栏「多选」按钮 → 切换会话多选模式（跨组件事件；侧栏折叠已改为 props 直连）
  useEffect(() => {
    const hMulti = () => (multiMode ? exitMulti() : enterMulti());
    window.addEventListener("NETON-toggle-multi", hMulti);
    return () => window.removeEventListener("NETON-toggle-multi", hMulti);
  });
  ...
  useEffect(() => {
    ...
    window.addEventListener("NETON-open-session", h);
    return () => window.removeEventListener("NETON-open-session", h);
