...
pub fn create(app: &tauri::AppHandle, ctx: &Arc<Ctx>) -> tauri::Result<()> {
    ...
    let tray = TrayIconBuilder::with_id("neton-tray")
        .icon(app.default_window_icon().unwrap().clone())
        .tooltip("NETON - 网络安全分析工具集")
        .menu(&menu)
        // 左键点击切换窗口显示
        ...
        .on_menu_event(move |app, event| match event.id.as_ref() {
            ...
            "quit" => {
                ...
                crate::audit::record(&quit_ctx, "local-app", "app.quit", "NETON", json!({ "via": "tray" }), true);
...
pub fn refresh(app: &tauri::AppHandle) {
    ...
    let _ = app.run_on_main_thread({
        ...
        move || {
            ...
            let Some(tray) = app.tray_by_id("neton-tray") else { return };
...
fn build_menu(
    ...
    let quit = MenuItem::with_id(app, "quit", "退出 NETON", true, None::<&str>)?;
...
pub fn show_main_window(app: &tauri::AppHandle) {
    if let Some(win) = app.get_webview_window("neton-main") {
        let _ = win.show();
