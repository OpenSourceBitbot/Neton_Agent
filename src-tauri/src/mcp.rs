...
pub async fn initialize(
    ...
    let (result, sid) = rpc(
        ...
        serde_json::json!({
            ...
            "capabilities": {},
            "clientInfo": { "name": "NETON", "version": env!("CARGO_PKG_VERSION") }
        }),
    )
    .await?;
...
mod tests {
    ...
    /// 仅在设置 NETON_FAKE_MCP_URL 环境变量时运行：
    ///   NETON_FAKE_MCP_URL=http://127.0.0.1:9801/mcp cargo test external_fake_mcp
    #[tokio::test]
    async fn test_external_fake_mcp() {
        let Ok(url) = std::env::var("NETON_FAKE_MCP_URL") else {
            eprintln!("跳过：未设置 NETON_FAKE_MCP_URL");
            return;
        };
        let http = client().unwrap();
        ...
        let echo = call_tool(&server, "echo", json!({ "message": "你好 NETON" })).await.unwrap();
        assert_eq!(echo["text"], "ECHO<你好 NETON>");
