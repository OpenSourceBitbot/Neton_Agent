...
//! - NETON_NO_GUARDIAN=1 环境变量可整体关闭（E2E / 调试用）
...
/// 守护进程命令行标记：`NETON --NETON-guardian <握手文件> <接力日志>`
pub const GUARDIAN_FLAG: &str = "--NETON-guardian";
...
/// 守护开关：默认开启，NETON_NO_GUARDIAN=1 关闭（E2E / 调试 / TUI）。
/// macOS 上默认关闭（用户 kill 后会被重拉，"关不掉"；如需守护可设 NETON_GUARDIAN=1 显式开启）。
pub fn enabled() -> bool {
    if cfg!(target_os = "macos") {
        return std::env::var("NETON_GUARDIAN").is_ok();
    }
    std::env::var("NETON_NO_GUARDIAN").is_err()
}
...
pub fn arm(ctx: &Arc<Ctx>) {
    ...
    if load_client_key(&ctx.data_dir).as_deref() != Some(key.as_str()) {
        ...
        drop(cfg);
        crate::audit::record(ctx, "local-app", "guardian.key_drift", "NETON", json!({ "resynced": true }), true);
    }
}
...
pub fn drain_log(ctx: &Arc<Ctx>) {
    ...
    if recorded > 0 {
        ...
        crate::audit::record(ctx, "guardian", "guardian.summary", "NETON", json!({ "events": recorded }), true);
    }
}

/// 守护进程主循环（本进程以 `NETON --NETON-guardian <握手文件> <日志>` 启动，不进入 GUI/TUI）。
...
mod tests {
    ...
    fn test_state_roundtrip() {
        let dir = std::env::temp_dir().join(format!("NETON-guardian-test-{}", std::process::id()));
    ...
    fn test_signed_roundtrip() {
        let dir = std::env::temp_dir().join(format!("NETON-guardian-sig-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("guardian.json");
    ...
    fn test_tamper_detected() {
        let dir = std::env::temp_dir().join(format!("NETON-guardian-tamper-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("guardian.json");
    ...
    fn test_legacy_plaintext_rejected() {
        let dir = std::env::temp_dir().join(format!("NETON-guardian-legacy-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("guardian.json");
    ...
    fn test_missing_and_wrong_key() {
        let dir = std::env::temp_dir().join(format!("NETON-guardian-miss-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("guardian.json");
    ...
    fn test_load_client_key() {
        let dir = std::env::temp_dir().join(format!("NETON-guardian-ck-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        // 正常读取
    ...
    fn test_tick_refresh_fallback_and_recover() {
        let dir = std::env::temp_dir().join(format!("NETON-guardian-tick-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("guardian.json");
    ...
    fn test_tick_refresh_key_rotation() {
        let dir = std::env::temp_dir().join(format!("NETON-guardian-rot-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("guardian.json");
        let log = dir.join("guardian.log");
    ...
    fn test_event_log_roundtrip() {
        let dir = std::env::temp_dir().join(format!("NETON-guardian-log-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn test_sha256_stable() {
        let dir = std::env::temp_dir().join(format!("NETON-guardian-sha-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
