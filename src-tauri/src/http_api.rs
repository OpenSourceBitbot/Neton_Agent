...
pub async fn restart_server(ctx: &Arc<Ctx>) -> Result<String, String> {
    ...
    let listener = match tokio::net::TcpListener::bind(&addr).await {
        ...
        Err(e) if e.kind() == std::io::ErrorKind::AddrInUse => {
            ...
            *ctx.port_switch.lock().unwrap() = Some(wanted_port);
            ...
            eprintln!("[NETON] port {wanted_port} in use, switched to {new_port}");
            listener
        }
    ...
    let task = tauri::async_runtime::spawn(async move {
        ...
        .await
        {
            eprintln!("[NETON] http server stopped: {e}");
        }
    });
    *ctx.server_task.lock().unwrap() = Some(task);
    // 设备凭证注册（幂等，先于服务与中继启动）：NETONsign-v2 验签材料依赖它。首次启动
    // 采集硬件指纹 + 公网 IP（公开 API 竞速 5s）+ 时间戳派生并落盘；失败不阻断服务
    if let Err(e) = crate::state::device::ensure_device_key(&ctx).await {
        eprintln!("[NETON] device register failed (channel verify degraded): {e}");
    }
...
pub fn build_router(ctx: Arc<Ctx>) -> Router {
    ...
    Router::new()
        ...
        // 调试接口（ADB 调试桥联动）：只读快照，供外部调试工具分析与控制 NETON
...
async fn health() -> Json<serde_json::Value> {
    Json(json!({ "ok": true, "service": "NETON", "time": chrono::Local::now().format("%Y-%m-%d %H:%M:%S").to_string() }))
}
...
/// NETON 支持的 MCP 协议版本（Streamable HTTP / JSON-RPC 2.0）。
        ...
        "initialize" => {
            ...
            (
                ...
                Some(rpc_ok(json!({
                    ...
                    "serverInfo": { "name": "NETON", "version": env!("CARGO_PKG_VERSION") }
                }))),
                Some(sid),
            )
        }
...
/// NETON 自身作为 MCP 服务器（Streamable HTTP / JSON-RPC 2.0，支持 2024-11-05 / 2025-03-26 / 2025-06-18 协商）：
/// 任何标准 MCP 客户端（Claude Desktop、Cherry Studio、MCP Inspector、NETON 自己的自动发现）都可接入 NETON 的全部启用工具。
...
/// 口径必须与 Worker 端许可盖章逐字符一致——NETON 端比对"真实来源 IP 的网段"与
/// Worker 盖章的许可网段（x-NETON-permit-pfx），实现差异会造成误杀正常请求
...
async fn auth(State(ctx): State<Arc<Ctx>>, req: Request, next: Next) -> Response {
    ...
    if let Err(status) = check_auth(&cfg, &path, &provided, &qkey, access_password) {
        if status == StatusCode::UNAUTHORIZED {
            ...
            let body = if reason == "client_key" {
                Json(json!({ "error": { "message": "无效的 API Key（NETON Client Key）", "type": "invalid_request_error", "code": "invalid_api_key" } }))
            } else {
        ...
        return (
            ...
            Json(json!({ "error": "NETON 尚未配置 Client Key，远程访问已禁用" })),
        )
            .into_response();
    }

    // 信道防护（NETONsign-v2）：x-NETON-via 头由本机中继循环回环时添加（直连请求没有），
    // 命中即视为"经云中继进入"→ 强制验签。第三方 App 无法算出合法签名，借道中继直接 403；
    // LAN 直连 / 本机访问不受影响（第三方 OpenAI 客户端兼容保留）。
    // 验签通过后返回经中继进入时附带的真实手机端 IP（x-NETON-client-ip，本机 poller 注入）
    let via_relay = req.headers().get("x-NETON-via").and_then(|v| v.to_str().ok()) == Some("relay");
    ...
    // （x-NETON-client-ip ← Worker cf-connecting-ip）与"许可签发时绑定的网段"（x-NETON-permit-pfx，
    // ← Worker 许可记录盖章，非现算）。官方中继在许可校验通过后必盖章：缺章或与真实
    // 来源不同段 = 中继实现被替换或被破坏，拒绝放行。两个头都只有本机 poller 从中继
    // 信封注入（PASS_HEADERS 白名单外），手机端无法伪造
    if via_relay {
        let cip = req.headers().get("x-NETON-client-ip").and_then(|v| v.to_str().ok()).unwrap_or("");
        let cpfx = req.headers().get("x-NETON-permit-pfx").and_then(|v| v.to_str().ok()).unwrap_or("");
    ...
    if via_relay && cfg.channel_guard {
        let rid = req.headers().get("x-NETON-rid").and_then(|v| v.to_str().ok()).unwrap_or("");
        let ts = req.headers().get("x-NETON-ts").and_then(|v| v.to_str().ok()).and_then(|v| v.parse::<i64>().ok()).unwrap_or(0);
        let nonce = req.headers().get("x-NETON-nonce").and_then(|v| v.to_str().ok()).unwrap_or("");
        let sign = req.headers().get("x-NETON-sign").and_then(|v| v.to_str().ok()).unwrap_or("");
        let now = chrono::Utc::now().timestamp();
        let check = crate::security::NETONsignCheck { ts, nonce: nonce.to_string(), sign: sign.to_string() };
        // 验签材料：设备凭证（NETONsign-v2）——未注册时用空材料（理论上不会发生：启动即注册）
        let material = crate::state::device::sig_material(cfg.device_key.as_deref().unwrap_or(""));
        // 验签 path = 本机请求路径（不含 query，与手机端签名口径一致）
        match crate::security::NETONsign_verify(&cfg.client_key, &material, rid, req.method().as_str(), req.uri().path(), &check, now) {
            Ok(()) => {
                relay_client_ip = req
                    .headers()
                    .get("x-NETON-client-ip")
                    .and_then(|v| v.to_str().ok())
    ...
    // 直连 / 未验签请求按 socket 地址——伪造头无法绕过黑名单（需先过 NETONsign）
...
/// 对话限速 / 并发计数用的客户端 IP：经验签的中继请求取真实手机端 IP（x-NETON-client-ip，
...
fn chat_client_ip(ctx: &Arc<Ctx>, headers: &HeaderMap, addr: &std::net::IpAddr) -> String {
    let via = headers.get("x-NETON-via").and_then(|v| v.to_str().ok()) == Some("relay");
    let guard = ctx.config.lock().unwrap().channel_guard;
    if via && guard {
        if let Some(v) = headers.get("x-NETON-client-ip").and_then(|v| v.to_str().ok()) {
...
struct RegisterReq {
    ...
    /// Agent 提供的回调端点，NETON 调用工具时会 POST 到该地址
    url: String,
}
...
// ==================== 调试接口（ADB 调试桥联动）====================
// 供 ADB（github.com/yxpil/ADB）等调试工具只读拉取 NETON 内部状态做分析。
...
/// MCP 服务器、会话/记忆/技能计数、远程访问配置。ADB「NETON」页的数据源。
...
async fn debug_quit(State(ctx): State<Arc<Ctx>>) -> Response {
    crate::audit::record(&ctx, "remote", "app.quit", "NETON", json!({ "via": "debug_api" }), true);
...
/// 远程对话：Agent 通过 HTTP 使用 NETON 的 AI 能力（含自写插件）
async fn remote_chat(
    ...
    if headers.get("x-NETON-via").and_then(|v| v.to_str().ok()) == Some("relay") {
...
// Base URL = http://<host>:<port>/v1，API Key = NETON 的 Client Key

#[derive(serde::Deserialize)]
struct OaiRequest {
    // 接收但忽略：实际路由始终由 NETON 激活的 Provider 决定
...
/// OpenAI messages → (NETON ChatMessage 列表, 图片列表)
...
async fn openai_models(State(ctx): State<Arc<Ctx>>) -> Response {
    let model = {
        ...
        cfg.active().map(|p| p.model.clone()).unwrap_or_else(|| "NETON".into())
    };
    Json(json!({
        "object": "list",
        "data": [
            { "id": model, "object": "model", "created": 0, "owned_by": "NETON" },
            { "id": "NETON", "object": "model", "created": 0, "owned_by": "NETON" }
        ]
    }))
    .into_response()
}
...
mod tests {
    ...
    fn test_key_hint_masks_secrets() {
        ...
        assert_eq!(key_hint("NETON_0123456789abcdef"), "NETON_01…(20)");
    ...
    fn test_auth_health_bypass() {
        ...
        let cfg = cfg_with_key("sk-NETON-test", Some("12345678"));
    ...
    fn test_auth_debug_endpoints_require_dual_auth() {
        ...
        for path in paths {
            assert!(
                check_auth(&cfg, path, "sk-NETON-test", "", "12345678").is_ok(),
                "{path} 双凭据应放行"
            );
            // 缺 Client Key → 401
            assert_eq!(
            ...
            assert_eq!(
                check_auth(&cfg, path, "sk-NETON-test", "", ""),
                Err(StatusCode::UNAUTHORIZED),
                "{path} 缺访问密码应 401"
            );
        }
        // ?key= 查询参数通道同样适用于调试端点
        assert!(check_auth(&cfg, "/api/debug/state", "", "sk-NETON-test", "12345678").is_ok());
    ...
    fn test_auth_bearer_key_accept_and_reject() {
        let cfg = cfg_with_key("sk-NETON-right", Some("12345678"));
        // 正确 Bearer + 密码 → 放行
        assert!(check_auth(&cfg, "/api/tools", "sk-NETON-right", "", "12345678").is_ok());
        // 错误 Bearer → 401
        assert_eq!(
            check_auth(&cfg, "/api/tools", "sk-NETON-wrong", "", "12345678"),
            Err(StatusCode::UNAUTHORIZED)
        );
        // 缺失 Bearer → 401
        assert_eq!(
    ...
    fn test_auth_access_password_enforced_only_on_api() {
        ...
        // /api/ 需要访问密码
        assert_eq!(
            check_auth(&cfg, "/api/chat", "sk-NETON-right", "", ""),
            Err(StatusCode::UNAUTHORIZED)
        );
        assert_eq!(
            check_auth(&cfg, "/api/chat", "sk-NETON-right", "", "wrong-pwd"),
            Err(StatusCode::UNAUTHORIZED)
        );
        assert!(check_auth(&cfg, "/api/chat", "sk-NETON-right", "", "12345678").is_ok());
        // 等价类：/v1/ 与 /mcp 豁免访问密码（OpenAI 客户端无法携带自定义头）
        assert!(check_auth(&cfg, "/v1/chat/completions", "sk-NETON-right", "", "").is_ok());
        assert!(check_auth(&cfg, "/mcp", "sk-NETON-right", "", "").is_ok());
    }

    #[test]
    fn test_auth_password_disabled_passes() {
        // 未启用密码校验（password_enabled=false）时直接通过
        let mut cfg = cfg_with_key("sk-NETON-right", None);
        cfg.password_enabled = false;
        assert!(check_auth(&cfg, "/api/chat", "sk-NETON-right", "", "").is_ok());
        // 启用但 access_password 为 None：一律拒绝（无法匹配）
        let mut cfg2 = cfg_with_key("sk-NETON-right", None);
        cfg2.password_enabled = true;
        assert_eq!(
            check_auth(&cfg2, "/api/chat", "sk-NETON-right", "", ""),
            Err(StatusCode::UNAUTHORIZED)
        );
    }
    ...
    fn test_actor_of_edges() {
        ...
        assert_eq!(actor_of("sk-NETON-12345678xyz"), "agent:it-12345"); // skip 4 位取 8 位
    }
