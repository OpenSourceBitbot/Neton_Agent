// yxpil · NETON
//! 信道安全模块：NETONsign-v1 请求签名 / NETON-Crypt v1 二维码加密块 / 敏感词审核 / nonce 重放缓存。
...
// ── NETONsign-v2：中继信道请求签名（只有本 App 的信道实现能算出合法签名）──
//
// canonical = "NETONsign-v2\n{rid}\n{ts}\n{nonce}\n{method}\n{path}\n{device_material}"
// device_material = 设备凭证的加盐 SHA256 前 16 hex（device.rs::sig_material）
// mac       = HMAC-SHA256(key = client_key, canonical)
// k         = SHA256("NETONsign-v2:" + client_key)                 // 32B 混淆密钥流
...
// 在验签端 canonical 长度/内容不匹配直接失效，防降级。真伪验证在 NETON 端完成。
//
// 验证端校验：ts 与本机时钟差 ≤ ±120s；nonce 未见过（防重放）；sign 常量时间比对。

pub const NETONSIGN_ALG: &str = "NETONsign-v2";
/// 签名时间戳容忍窗口（秒）：覆盖设备间时钟漂移，同时限制截获签名的有效时长
pub const NETONSIGN_TS_WINDOW: i64 = 120;
...
pub fn NETONsign(client_key: &str, device_material: &str, rid: &str, ts: i64, nonce: &str, method: &str, path: &str) -> String {
    ...
    let k = Sha256::digest(format!("{NETONSIGN_ALG}:{client_key}").as_bytes());
    let mut out = [0u8; 32];
    for i in 0..32 {
...
#[cfg(test)]
pub fn NETONcrypt_decrypt(blob: &str) -> Result<String, &'static str> {
    use base64::Engine;
    let raw = blob
...
/// 窗口与 NETONsign 时钟容忍一致：过期签名本身已被拒，nonce 无需记忆更久
...
mod tests {
    ...
    #[test]
    fn NETONsign_roundtrip_and_tamper() {
        let key = "NETON_test_key";
        let mat = "ab12";
        let s = NETONsign(key, mat, "rid3333", 1_000_000, "nonce1234", "POST", "/api/chat");
        assert_eq!(s.len(), 64);
        let ok = NETONsign_verify(
            key,
            mat,
            "rid3333",
            "POST",
            "/api/chat",
            &NETONsignCheck { ts: 1_000_000, nonce: "nonce1234".into(), sign: s.clone() },
            1_000_060,
        );
        assert!(ok.is_ok(), "{ok:?}");
        // 篡改 method / path / rid / ts / nonce / sign / 设备材料 任意一项都必须失败
        assert!(NETONsign_verify(key, mat, "rid3333", "GET", "/api/chat", &NETONsignCheck { ts: 1_000_000, nonce: "nonce1234".into(), sign: s.clone() }, 1_000_060).is_err());
        assert!(NETONsign_verify(key, mat, "rid3333", "POST", "/api/qr", &NETONsignCheck { ts: 1_000_000, nonce: "nonce1234".into(), sign: s.clone() }, 1_000_060).is_err());
        assert!(NETONsign_verify(key, mat, "rid4444", "POST", "/api/chat", &NETONsignCheck { ts: 1_000_000, nonce: "nonce1234".into(), sign: s.clone() }, 1_000_060).is_err());
        assert!(NETONsign_verify(key, mat, "rid3333", "POST", "/api/chat", &NETONsignCheck { ts: 1_000_000 + 1000, nonce: "nonce1234".into(), sign: s.clone() }, 1_000_060).is_err());
        assert!(NETONsign_verify(key, mat, "rid3333", "POST", "/api/chat", &NETONsignCheck { ts: 1_000_000, nonce: "nonce5678".into(), sign: s.clone() }, 1_000_060).is_err());
        let mut bad = s.clone().into_bytes();
        bad[3] = if bad[3] == b'a' { b'b' } else { b'a' };
        assert!(NETONsign_verify(key, mat, "rid3333", "POST", "/api/chat", &NETONsignCheck { ts: 1_000_000, nonce: "nonce1234".into(), sign: String::from_utf8(bad).unwrap() }, 1_000_060).is_err());
        // 过期
        assert!(NETONsign_verify(key, mat, "rid3333", "POST", "/api/chat", &NETONsignCheck { ts: 1_000_000, nonce: "nonce1234".into(), sign: s.clone() }, 1_000_000 + NETONSIGN_TS_WINDOW + 5).is_err());
        // 错误 key
        assert!(NETONsign_verify("other", mat, "rid3333", "POST", "/api/chat", &NETONsignCheck { ts: 1_000_000, nonce: "nonce1234".into(), sign: s.clone() }, 1_000_060).is_err());
        // 设备材料不同（未注册设备 / 换设备）→ 签名失效（v2 防共用核心）
        assert!(NETONsign_verify(key, "cd34", "rid3333", "POST", "/api/chat", &NETONsignCheck { ts: 1_000_000, nonce: "nonce1234".into(), sign: s }, 1_000_060).is_err());
    }

    #[test]
    fn NETONsign_v1_signature_rejected() {
        // 防降级：老版本 v1 签名（canonical 无设备材料段）在 v2 验签端必须失效
        let key = "NETON_test_key";
        let v1_like = NETONsign(key, "", "rid3333", 1_000_000, "nonce1234", "POST", "/api/chat");
        // 即使把空材料签出的值拿来验证别的材料口径，也因材料不匹配而失败
        assert!(NETONsign_verify(key, "ab12", "rid3333", "POST", "/api/chat", &NETONsignCheck { ts: 1_000_000, nonce: "nonce1234".into(), sign: v1_like.clone() }, 1_000_060).is_err());
        // 同理：v2 签名换材料必失败已由 roundtrip 覆盖；这里再断言空材料自身自洽
        assert!(NETONsign_verify(key, "", "rid3333", "POST", "/api/chat", &NETONsignCheck { ts: 1_000_000, nonce: "nonce1234".into(), sign: v1_like }, 1_000_060).is_ok());
    }

    #[test]
    fn NETONcrypt_roundtrip_and_tamper() {
        let msg = r#"{"v":2,"key":"NETON_x","rid":"ab","pwd":"12345678"}"#;
        let enc = NETONcrypt_encrypt(msg);
        assert!(enc.starts_with(CRYPT_PREFIX));
        assert!(!enc.contains("NETON_x"), "ciphertext must not leak plaintext key");
        // 每次盐随机：两段密文不同但都可解
        let enc2 = NETONcrypt_encrypt(msg);
        assert_ne!(enc, enc2);
        assert_eq!(NETONcrypt_decrypt(&enc).unwrap(), msg);
        assert_eq!(NETONcrypt_decrypt(&enc2).unwrap(), msg);
        // 篡改一位 → 解不出原文
        let mut raw = enc.clone().into_bytes();
        let last = raw.len() - 2;
        raw[last] = if raw[last] == b'A' { b'B' } else { b'A' };
        let dec = NETONcrypt_decrypt(&String::from_utf8(raw).unwrap());
        assert!(dec.is_err() || dec.unwrap() != msg);
        assert!(NETONcrypt_decrypt("XXXX:abc").is_err());
    }
