...
/// 工具类型：
/// - Builtin: NETON 出厂内置实现（shell / write_file / plan / edit / add_tool）
/// - Remote: Agent 注册的回调端点，NETON 调用该 URL 执行
...
pub fn builtin_tools() -> Vec<ToolDef> {
    ...
    vec![
        ...
        mk(
            "netsec.port_scan",
            "port_scan",
            "Scan TCP ports on a target host to discover open services. Returns open ports with service names and response times. Use for network reconnaissance and security auditing.",
            serde_json::json!({
                "type": "object",
                "properties": {
                    "target": { "type": "string", "description": "Target IP address or hostname to scan" },
                    "ports": { "type": "string", "description": "Comma-separated port numbers, or 'common' for top 20 ports, or 'full' for 1-1024 (default: common)" },
                    "timeout_ms": { "type": "integer", "description": "Timeout per port in milliseconds (default 1000)" }
                },
                "required": ["target"]
            }),
            "netsec_port_scan",
        ...
        mk(
            "netsec.web_vuln_scan",
            "web_vuln_scan",
            "Scan a website for common web vulnerabilities including XSS, CSRF, SQL injection, file inclusion, command injection, XXE, SSRF, open redirect, and clickjacking. Returns findings with severity levels and remediation advice.",
            serde_json::json!({
                "type": "object",
                "properties": {
                    "url": { "type": "string", "description": "Target website URL to scan for vulnerabilities" },
                    "deep": { "type": "boolean", "description": "Deep scan mode: includes subdomain enumeration and directory brute force (default: false)" }
                },
                "required": ["url"]
            }),
            "netsec_web_vuln_scan",
        ),
        mk(
            "netsec.site_analyze",
            "site_analyze",
            "Comprehensive site intelligence gathering: CMS detection (20+ CMS), server fingerprinting, tech stack identification (frontend/backend/database/CDN/WAF), SSL certificate analysis, and security scoring.",
            serde_json::json!({
                "type": "object",
                "properties": {
                    "url": { "type": "string", "description": "Target website URL or domain to analyze" },
                    "deep": { "type": "boolean", "description": "Deep analysis: includes subdomain discovery and directory probing (default: false)" }
                },
                "required": ["url"]
            }),
            "netsec_site_analyze",
        ...
        mk(
            "netsec.cve_search",
            "cve_search",
            "Search for CVE vulnerabilities by keyword, product name, or CVE ID. Returns vulnerability details including severity (CVSS score), description, affected versions, and references.",
            serde_json::json!({
                "type": "object",
                "properties": {
                    "query": { "type": "string", "description": "Search query: keyword, product name, or CVE ID (e.g. CVE-2024-1234)" },
        ...
        mk(
            "netsec.domain_analysis",
            "domain_analyze",
            "Analyze a domain: DNS records (A, AAAA, MX, NS, TXT, CNAME, SOA), WHOIS information, subdomain enumeration, and domain age calculation.",
            serde_json::json!({
                "type": "object",
                "properties": {
                    "domain": { "type": "string", "description": "Domain name to analyze (without http://)" }
                },
                "required": ["domain"]
            }),
            "netsec_domain_analyze",
        ),
        mk(
            "netsec.web_analyze",
            "web_analyze",
            "Analyze a single web page: extract title, meta tags, all links, forms, cookies, security headers, technology fingerprint, and sensitive information detection.",
            serde_json::json!({
                "type": "object",
                "properties": {
                    "url": { "type": "string", "description": "Web page URL to analyze" },
                    "deep": { "type": "boolean", "description": "Deep analysis with more thorough extraction (default: false)" }
                },
                "required": ["url"]
            }),
            "netsec_web_analyze",
        ...
        mk(
            "netsec.hash_compute",
            "hash_compute",
            "Compute cryptographic hash of a text string. Supports MD5, SHA-1, SHA-256, SHA-512, SHA-384, SHA-224, CRC32, and HMAC.",
            serde_json::json!({
                "type": "object",
                "properties": {
                    "text": { "type": "string", "description": "Text to compute hash from" },
                    "algorithm": { "type": "string", "description": "Hash algorithm: md5, sha1, sha256, sha512, sha384, sha224, crc32, or all (default: sha256)" }
                },
                "required": ["text"]
            }),
            "netsec_hash_compute",
        ...
        mk(
            "netsec.encode_decode",
            "encode_decode",
            "Encode or decode text in various formats: base64, base32, hex, url, html, unicode, rot13, caesar cipher, morse code, binary, octal.",
            serde_json::json!({
                "type": "object",
                "properties": {
                    "input": { "type": "string", "description": "Text to encode or decode" },
                    "format": { "type": "string", "description": "Encoding format: base64, base32, hex, url, html, unicode, rot13, caesar, morse, binary, octal (default: base64)" },
                    "decode": { "type": "boolean", "description": "Set true to decode, false to encode (default: false)" },
                    "shift": { "type": "integer", "description": "Caesar cipher shift amount (1-25, default 3, only for caesar format)" }
                },
                "required": ["input", "format"]
            }),
            "netsec_encode_decode",
        ),
        mk(
            "netsec.password_strength",
            "password_strength",
            "Analyze password strength and detect weak passwords. Checks length, character variety, common passwords, dictionary words, patterns, and gives a strength score with improvement suggestions.",
            serde_json::json!({
                "type": "object",
                "properties": {
            ...
            "netsec_password_strength",
        ),
        mk(
            "netsec.arp_scan",
            "arp_scan",
            "Scan the local network using ARP to discover connected devices. Returns device list with IP, MAC address, manufacturer, and device type. Only works on the local subnet.",
            serde_json::json!({
                "type": "object",
                "properties": {
                    "subnet": { "type": "string", "description": "Target subnet in CIDR notation, e.g. 192.168.1.0/24 (leave empty for auto-detect)" },
                    "timeout_ms": { "type": "integer", "description": "Timeout per host in milliseconds (default 500)" }
                },
                "required": []
            }),
            "netsec_arp_scan",
...
pub fn remove(ctx: &Arc<crate::state::Ctx>, id: &str) -> Result<String, String> {
    let mut tools = ctx.tools.lock().unwrap();
    // 内置工具是 NETON 出厂能力，不允许删除（用户 UI 与 AI delete_tool 都走这里）
    if tools
        .iter()
...
async fn builtin_invoke(
    ...
    match handler {
        ...
        // ── 网络安全工具 handlers ──
        "netsec_port_scan" => {
            let target = params.get("target").and_then(|v| v.as_str()).unwrap_or_default().to_string();
            let ports = params.get("ports").and_then(|v| v.as_str()).unwrap_or("common").to_string();
            let timeout_ms = params.get("timeout_ms").and_then(|v| v.as_u64()).unwrap_or(1000);
            if target.is_empty() {
                return Err("port_scan requires the target parameter".into());
            }
            use crate::netsec::port_scanner::{PortScanConfig, ScanMode};
            let (mode, start_port, end_port) = match ports.to_lowercase().as_str() {
                "common" | "quick" => (ScanMode::Quick, 1u16, 1024u16),
                "full" | "all" => (ScanMode::TcpConnect, 1u16, 1024u16),
                s if s.contains('-') => {
                ...
                s if s.contains(',') => {
                    ...
                    let ps: Vec<u16> = s
                        ...
                        .collect();
                    let start = ps.iter().min().copied().unwrap_or(1);
                    let end = ps.iter().max().copied().unwrap_or(1024);
                    (ScanMode::TcpConnect, start, end)
                }
                _ => (ScanMode::Quick, 1u16, 1024u16),
            };
            ...
            let result = crate::netsec::port_scanner::scan_ports(config).await;
            match result {
                Ok(json_str) => Ok(serde_json::from_str(&json_str).unwrap_or(serde_json::json!({ "result": json_str }))),
                Err(e) => Err(e),
            }
        }
        "netsec_web_vuln_scan" => {
            ...
            if url.is_empty() {
                return Err("web_vuln_scan requires the url parameter".into());
            }
            use crate::netsec::vuln_scanner::VulnScanConfig;
            let config = VulnScanConfig {
                target_url: url.clone(),
                scan_types: vec!["all".to_string()],
                depth: if deep { 3 } else { 1 },
                timeout_ms: 8000,
                delay_ms: if deep { 200 } else { 500 },
                test_param: None,
            ...
            let result = crate::netsec::vuln_scanner::scan_vulnerabilities(url, config);
            match result {
                Ok(json_str) => Ok(serde_json::from_str(&json_str).unwrap_or(serde_json::json!({ "result": json_str }))),
                Err(e) => Err(e),
            }
        ...
        "netsec_site_analyze" => {
            ...
            let deep = params.get("deep").and_then(|v| v.as_bool()).unwrap_or(false);
            if url.is_empty() {
                return Err("site_analyze requires the url parameter".into());
            }
            use crate::netsec::site_info::SiteInfoConfig;
            let config = SiteInfoConfig {
                deep_scan: deep,
                enable_subdomain_scan: deep,
                enable_dir_scan: deep,
                ..Default::default()
            };
            let result = crate::netsec::site_info::analyze_site(url, config);
            match result {
                Ok(json_str) => Ok(serde_json::from_str(&json_str).unwrap_or(serde_json::json!({ "result": json_str }))),
                Err(e) => Err(e),
            }
        ...
        "netsec_cve_search" => {
            let query = params.get("query").and_then(|v| v.as_str()).unwrap_or_default().to_string();
            let limit = params.get("limit").and_then(|v| v.as_u64()).unwrap_or(10) as u32;
            if query.is_empty() {
                return Err("cve_search requires the query parameter".into());
            }
            use crate::netsec::cve_search::CveSearchConfig;
            let config = if query.starts_with("CVE-") || query.starts_with("cve-") {
                CveSearchConfig {
                    cve_id: Some(query.clone()),
                    limit,
                    ..Default::default()
                }
            } else {
            ...
            let result = crate::netsec::cve_search::search_cve(config).await;
            match result {
                Ok(json_str) => Ok(serde_json::from_str(&json_str).unwrap_or(serde_json::json!({ "result": json_str }))),
                Err(e) => Err(e),
            }
        ...
        "netsec_domain_analyze" => {
            let domain = params.get("domain").and_then(|v| v.as_str()).unwrap_or_default().to_string();
            if domain.is_empty() {
                return Err("domain_analyze requires the domain parameter".into());
            }
            let result = crate::netsec::domain_analysis::analyze_domain(domain).await;
            match result {
                Ok(json_str) => Ok(serde_json::from_str(&json_str).unwrap_or(serde_json::json!({ "result": json_str }))),
                Err(e) => Err(e),
            }
        ...
        "netsec_web_analyze" => {
            ...
            let deep = params.get("deep").and_then(|v| v.as_bool()).unwrap_or(false);
            if url.is_empty() {
                return Err("web_analyze requires the url parameter".into());
            }
            use crate::netsec::web_analyzer::WebAnalysisOptions;
            let options = WebAnalysisOptions {
                detect_sensitive_info: deep,
                detect_tech_fingerprint: true,
                detect_waf: true,
                ..Default::default()
            };
            let result = crate::netsec::web_analyzer::analyze_webpage(url, options);
            match result {
                Ok(json_str) => Ok(serde_json::from_str(&json_str).unwrap_or(serde_json::json!({ "result": json_str }))),
                Err(e) => Err(e),
            }
        ...
        "netsec_hash_compute" => {
            ...
            let algorithm = params.get("algorithm").and_then(|v| v.as_str()).unwrap_or("sha256").to_string();
            if text.is_empty() {
                return Err("hash_compute requires the text parameter".into());
            }
            let result = if algorithm == "all" {
                crate::netsec::hash_crypto::compute_multiple_hashes(text)
            } else {
                crate::netsec::hash_crypto::compute_hash(text, algorithm)
            };
            match result {
                Ok(json_str) => Ok(serde_json::from_str(&json_str).unwrap_or(serde_json::json!({ "result": json_str }))),
                Err(e) => Err(e),
        ...
        "netsec_encode_decode" => {
            ...
            let format = params.get("format").and_then(|v| v.as_str()).unwrap_or("base64").to_string();
            let decode = params.get("decode").and_then(|v| v.as_bool()).unwrap_or(false);
            if input.is_empty() {
                return Err("encode_decode requires the input parameter".into());
            }
            let result = crate::netsec::hash_crypto::encode_decode(input, format, decode);
            match result {
                Ok(json_str) => Ok(serde_json::from_str(&json_str).unwrap_or(serde_json::json!({ "result": json_str }))),
                Err(e) => Err(e),
            }
        ...
        "netsec_password_strength" => {
            ...
            if password.is_empty() {
                return Err("password_strength requires the password parameter".into());
            }
            let result = crate::netsec::weak_password::analyze_password(password);
            match result {
                Ok(json_str) => Ok(serde_json::from_str(&json_str).unwrap_or(serde_json::json!({ "result": json_str }))),
                Err(e) => Err(e),
            }
        ...
        "netsec_arp_scan" => {
            let subnet = params.get("subnet").and_then(|v| v.as_str()).unwrap_or_default().to_string();
            let timeout_ms = params.get("timeout_ms").and_then(|v| v.as_u64()).unwrap_or(500);
            use crate::netsec::arp_scanner::ArpScanConfig;
            let network = if subnet.is_empty() {
            ...
            let result = crate::netsec::arp_scanner::scan_arp(config).await;
            match result {
                Ok(json_str) => Ok(serde_json::from_str(&json_str).unwrap_or(serde_json::json!({ "result": json_str }))),
                Err(e) => Err(e),
            }
