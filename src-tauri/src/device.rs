...
//! 稳定锚点：并入信道签名材料（NETONsign-v2），被中继转发的请求必须能算出 v2 签名。
//!
//! 语义约定：
...
pub const DEVICE_KEY_PREFIX: &str = "NETONdev_";
/// 签名材料派生盐（与 sign.cjs / 手机端实现约定一致）
pub const SIG_MATERIAL_SALT: &str = "NETONdev-material:";
...
mod tests {
    ...
    fn key_stable_and_ts_bound() {
        ...
        assert!(k1.starts_with("NETONdev_") && k1.len() == 32 + DEVICE_KEY_PREFIX.len());
    }

    #[test]
    fn sig_material_stable() {
        let a = sig_material("NETONdev_abcdef");
        let b = sig_material("NETONdev_abcdef");
        let c = sig_material("NETONdev_other");
        assert_eq!(a, b);
        assert_ne!(a, c);
        assert_eq!(a.len(), 16);
    }
