...
pub fn run(
    ...
    // NETON toolhomes：AI 代码的统一工作目录与环境（venv / NODE_PATH）
...
fn run_interpreted(
    ...
    let tmp = std::env::temp_dir().join(format!("NETON_{}.{ext}", uuid::Uuid::new_v4().simple()));
...
mod tests {
    ...
    fn test_write_private_mode_0600() {
        use std::os::unix::fs::PermissionsExt;
        let path = std::env::temp_dir().join(format!("NETON_test_{}", uuid::Uuid::new_v4().simple()));
