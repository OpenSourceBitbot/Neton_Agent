...
//
// IPC（全部只绑 127.0.0.1，随机 128-NETON token 鉴权，经环境变量下发给 worker）：
...
async fn start_event_sink(ctx: &Arc<Ctx>) -> Result<String, String> {
    ...
    async fn receive(
        ...
        if headers.get("x-NETON-worker-token").and_then(|v| v.to_str().ok()) != Some(token.as_str()) {
...
async fn ping() -> Result<PingInfo, String> {
    let v: serde_json::Value = http_quick()
        ...
        .header("x-NETON-worker-token", token())
        .json(&json!({}))
        .send()
        .await
        .map_err(|e| e.to_string())?
...
/// 拉起 worker 子进程并完成握手（stdout 首行 NETON_WORKER_READY <port>）
async fn ensure_spawned(ctx: &Arc<Ctx>) -> Result<(), String> {
    ...
    cmd.arg(WORKER_FLAG)
        .env("NETON_WORKER_TOKEN", token())
        .env("NETON_HOST_EVENT_URL", sink)
        .env("NETON_HOST_PID", std::process::id().to_string())
        .env("NETON_DATA_DIR", ctx.data_dir.to_string_lossy().to_string())
            ...
            match std::io::Read::read(&mut stdout, &mut buf) {
                ...
                Ok(n) => {
                    ...
                    if let Some(pos) = acc.iter().position(|&b| b == b'\n') {
                        ...
                        let _ = tx.send(match line.strip_prefix("NETON_WORKER_READY ") {
...
pub async fn serve(ctx: Arc<Ctx>) -> Result<(), String> {
    ...
    let expect_token = std::env::var("NETON_WORKER_TOKEN").unwrap_or_default();
    if expect_token.is_empty() {
        return Err("NETON_WORKER_TOKEN missing; refusing to serve".into());
    }
    // 宿主退出检测：宿主 PID 消失后自杀，防孤儿泄漏
    if let Ok(pid) = std::env::var("NETON_HOST_PID") {
        tauri::async_runtime::spawn(async move {
            loop {
    ...
    EV_TX.set(tx).ok();
    let sink = std::env::var("NETON_HOST_EVENT_URL").unwrap_or_default();
    ...
    tokio::spawn(async move {
        ...
        while let Some((name, payload)) = rx.recv().await {
            ...
            let _ = client
                .post(&sink)
                .header("x-NETON-worker-token", &sink_token)
    ...
    println!("NETON_WORKER_READY {port}");
    ...
    async fn auth(
        ...
        if headers.get("x-NETON-worker-token").and_then(|v| v.to_str().ok()) != Some(st.token.as_str()) {
