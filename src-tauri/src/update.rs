...
/// 自动更新：检测（镜像 → GitHub API，NETON_FAKE_UPDATE_URL 测试注入在首位）
...
/// 检测最新版本。NETON_FAKE_UPDATE_URL 指向的源优先（e2e 测试注入）。
...
pub async fn fetch_latest() -> Result<LatestInfo, String> {
    ...
    if let Ok(fake) = std::env::var("NETON_FAKE_UPDATE_URL") {
        sources.push(fake);
    }
    sources.push("https://yxpil.github.io/NETON/latest.json".into());
    ...
    if let Ok(v) = client
        .get("https://api.github.com/repos/yxpil/NETON/releases/latest")
        .header("User-Agent", "NETON-Agent")
        .send()
        .await
    {
...
fn trusted_asset_url(url: &str) -> bool {
    trusted_asset_url_impl(url, std::env::var_os("NETON_FAKE_UPDATE_URL").is_some())
}

/// 纯函数实现。allow_loopback 仅由 e2e 注入变量 NETON_FAKE_UPDATE_URL 开启：
...
pub fn pick_asset(latest: &serde_json::Value) -> Option<(String, String)> {
        ...
        .and_then(|v| v.as_str())
    {
        if trusted_asset_url(u) {
            let name = u.rsplit('/').next().unwrap_or("NETON-update.bin").to_string();
    ...
    let name = match key {
        "windows-x64" => format!("NETON_{v}_x64-setup.exe"),
        "windows-arm64" => format!("NETON_{v}_aarch64-setup.exe"),
        "macos-arm64" => format!("NETON_{v}_aarch64-app.zip"),
        "macos-x64" => format!("NETON_{v}_x64-app.zip"),
        "linux-x64" => format!("NETON_{v}_amd64.AppImage"),
        "linux-arm64" => format!("NETON_{v}_aarch64.AppImage"),
        _ => return None,
    };
    Some((
        name.clone(),
        format!("https://github.com/yxpil/NETON/releases/download/v{v}/{name}"),
    ))
}
...
pub fn apply_update(ctx: &Arc<Ctx>, respawn: bool) -> Result<String, String> {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        Ok("安装器已启动，NETON 即将退出并完成更新".into())
    }

    #[cfg(target_os = "macos")]
    {
        // 解包 app.zip → 找到 NETON.app → 整体替换当前 bundle（先挪走旧的作备份）
        ...
        let new_app = find_app_dir(&extract).ok_or("解包内容中未找到 NETON.app")?;
        let cur_app = exe
            .ancestors()
        ...
        if respawn {
            let _ = std::process::Command::new(&cur_app.join("Contents/MacOS/NETON")).spawn();
        }
        Ok("更新完成".into())
    }
    ...
    {
        // AppImage 单文件换装
        std::fs::copy(&file, upgrade_dir(ctx).join("NETON.new"))
            .map_err(|e| format!("准备失败: {e}"))?;
        let staged = upgrade_dir(ctx).join("NETON.new");
...
/// 在解包目录里递归找 NETON.app（zip 内可能有一层目录包裹）
...
mod tests {
    ...
    fn pick_asset_from_map() {
        ...
        let latest = serde_json::json!({
            "version": "9.9.9",
            "assets": {
                "windows-x64": "https://github.com/yxpil/NETON/releases/download/v9.9.9/NETON_9.9.9_x64-setup.exe",
                "windows-arm64": "https://github.com/yxpil/NETON/releases/download/v9.9.9/NETON_9.9.9_aarch64-setup.exe",
                "macos-arm64": "https://github.com/yxpil/NETON/releases/download/v9.9.9/NETON_9.9.9_aarch64-app.zip",
                "macos-x64": "https://github.com/yxpil/NETON/releases/download/v9.9.9/NETON_9.9.9_x64-app.zip",
                "linux-x64": "https://github.com/yxpil/NETON/releases/download/v9.9.9/NETON_9.9.9_amd64.AppImage",
                "linux-arm64": "https://github.com/yxpil/NETON/releases/download/v9.9.9/NETON_9.9.9_aarch64.AppImage",
            }
        });
    ...
    fn pick_asset_rejects_untrusted_host() {
        ...
        for evil in [
            "http://x/NETON_9.9.9_x64-setup.exe",
            "https://evil.com/NETON_9.9.9_x64-setup.exe",
            "https://github.com.evil.com/NETON_9.9.9_x64-setup.exe",
            "ftp://github.com/NETON_9.9.9_x64-setup.exe",
        ] {
            let latest = serde_json::json!({ "version": "9.9.9", "assets": { "windows-x64": evil } });
            if let Some((_, url)) = pick_asset(&latest) {
                assert!(url.starts_with("https://github.com/yxpil/NETON/releases/"), "evil={evil} url={url}");
            }
        }
    }
    ...
    fn pick_asset_fallback_constructs_github_url() {
        ...
        assert!(url == format!("https://github.com/yxpil/NETON/releases/download/v0.5.0/{name}"));
    ...
    fn trusted_url_loopback_only_with_fake_env() {
        ...
        assert!(trusted_asset_url_impl("https://github.com/yxpil/NETON/releases/download/v1/x", false));
        assert!(trusted_asset_url_impl("https://osbt.space/NETON.dmg", false));
    }

    #[test]
    fn replace_binary_swaps_and_keeps_backup() {
        let dir = std::env::temp_dir().join(format!("NETON-swap-test-{}", std::process::id()));
    ...
    fn replace_binary_rolls_back_on_failure() {
        let dir = std::env::temp_dir().join(format!("NETON-swap-rb-{}", std::process::id()));
        let _ = std::fs::create_dir_all(&dir);
        let target = dir.join("app.bin");
        std::fs::write(&target, b"OLD").unwrap();
