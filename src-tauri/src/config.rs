...
pub struct Config {
    ...
    /// 信道防护：经云中继进来的请求必须携带合法 NETONsign-v2 签名（只有完成设备注册的
    /// 本 App 信道实现能算出），第三方 App 借道中继直接 403。LAN 直连不受影响
    #[serde(default = "default_true")]
    pub channel_guard: bool,
    /// 设备凭证（NETONdev_*）：硬件指纹+公网IP+注册时间戳派生（device.rs），账号凭证级
...
mod tests {
    ...
    fn test_default_remote_disabled() {
        ...
        assert!(cfg.client_key.starts_with("NETON_"));
    ...
    fn test_load_preserves_enabled() {
        let dir = std::env::temp_dir().join(format!("NETON-cfg-test-{}", std::process::id()));
        fs::create_dir_all(&dir).unwrap();
        let path = dir.join("config.json");
        fs::write(
            &path,
            r#"{"remote_enabled":true,"host":"0.0.0.0","port":8600,"client_key":"NETON_x","revision":3}"#,
        )
        .unwrap();
    ...
    fn test_verify_client_key() {
        let cfg = Config {
            client_key: "NETON_ab12ab12ab12ab12ab12ab12ab12ab12".into(),
            ..Config::default()
        };
        assert!(cfg.verify_client_key("NETON_ab12ab12ab12ab12ab12ab12ab12ab12"));
        assert!(!cfg.verify_client_key("NETON_ab12ab12ab12ab12ab12ab12ab12ab13"));
        assert!(!cfg.verify_client_key("NETON_"));
