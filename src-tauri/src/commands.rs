...
/// 无界面模式（NETON_HEADLESS=1）：E2E/专项测试用，窗口保持隐藏不弹到前台
#[tauri::command]
pub fn is_headless() -> bool {
    std::env::var("NETON_HEADLESS")
...
pub async fn qr_payload(ctx: &Arc<Ctx>) -> Result<serde_json::Value, String> {
    ...
    // 信道签名算法标识：手机端据此实现配套的请求签名（NETONsign-v2，材料含设备凭证）
    payload["alg"] = json!(crate::security::NETONSIGN_ALG);
    // 加密块（NETON-Crypt v1）：把整个 payload JSON 加密成 NETON1: 密文——二维码图只编密文，
    // 普通扫码器/截图外泄读不出 client_key / rid 等连接凭据，只有本 App（内置主密钥）能解
    let plain = serde_json::to_string(&payload).map_err(|e| e.to_string())?;
    payload["enc"] = json!(crate::security::NETONcrypt_encrypt(&plain));
    Ok(payload)
}
...
pub async fn get_remote_qr(state: State<'_, Arc<Ctx>>) -> Result<serde_json::Value, String> {
    ...
    // 二维码图只编加密块（NETON1: 密文）：截图/扫码器读不出明文凭据，
...
fn notify_done(app: &tauri::AppHandle, ctx: &Arc<Ctx>, session_id: &str, messages: &[crate::ai::ChatMessage]) {
    ...
    let title = ctx
        ...
        .map(|s| s.title.clone())
        .unwrap_or_else(|| "NETON".to_string());
    let reply = messages
        .iter()
        .rev()
...
pub fn quit_app(state: State<'_, Arc<Ctx>>) -> Result<serde_json::Value, String> {
    let ctx = ctx(state);
    crate::audit::record(&ctx, "local-app", "app.quit", "NETON", json!({ "via": "ui" }), true);
    // 正常退出：先通知守护进程不要接力拉起
...
/// 进程环境变量，仅靠 NETON_DATA_DIR 会在提权后丢失。
fn relaunch_with_elevation(exe: &std::path::Path, data_dir: &str, elevate: bool) -> Result<(), String> {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        // explorer 不透传参数；NETON_DATA_DIR 覆盖仅用于测试环境，正常数据目录为标准位置不受影响
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        if elevate {
            ...
            // 命令里同时传递 --data-dir 和 NETON_DATA_DIR（环境在 root shell 里会清空）；
            // 最重要：osascript 成功后验证新进程是否真的活着，没活就返回错误，
            // 调用方会保持当前进程运行、不退出老进程（用户不会丢失当前会话）
            let script = format!(
                "do shell script \"NETON_DATA_DIR='{d}' '{e}' --data-dir '{d}' >/dev/null 2>&1 &\" with administrator privileges",
                d = sh_d,
                e = sh_e
            );
        ...
        std::process::Command::new("su")
            .args(["-l", &user, "-c", &format!("NETON_DATA_DIR='{d}' nohup '{e}' >/dev/null 2>&1 &", d = sh_d, e = sh_e)])
            .spawn()
            .map(|_| ())
            .map_err(|e| format!("降权启动失败: {e}"))
    }
...
/// 安装 `NETON` 命令到终端 PATH：macOS/Linux 优先 /usr/local/bin 符号链接（无权限回退 ~/.local/bin），
/// Windows 写 NETON.cmd 到用户 PATH 自带的 %LOCALAPPDATA%\Microsoft\WindowsApps（start /wait 等待 GUI 进程）。
/// NETON_CLI_DIR 环境变量可覆盖安装目录（E2E 测试用）。
pub fn install_cli_impl(ctx: &Arc<Ctx>) -> Result<serde_json::Value, String> {
    ...
    if let Ok(dir) = std::env::var("NETON_CLI_DIR") {
...
fn install_cli_into(dir: &std::path::Path, exe: &std::path::Path) -> Result<serde_json::Value, String> {
    ...
    #[cfg(target_os = "windows")]
    {
        let launcher = dir.join("NETON.cmd");
        let script = format!("@echo off\r\nstart \"\" /b /wait \"{}\" tui %*\r\n", exe.display());
        std::fs::write(&launcher, script).map_err(|e| format!("写入启动器失败: {e}"))?;
        Ok(serde_json::json!({ "path": launcher.display().to_string(), "hint": "重新打开终端后生效，运行 NETON tui 进入终端模式" }))
    }
    #[cfg(not(target_os = "windows"))]
    {
        let link = dir.join("NETON");
...
/// NETON_FAKE_UPDATE_URL 环境变量可将检测源替换为测试注入的地址（e2e 用）。
#[tauri::command]
...
mod tests {
    ...
    fn test_crash_tail_roundtrip() {
        ...
        let dir = std::env::temp_dir().join(format!("NETON-crash-{}", std::process::id()));
    ...
    fn test_clean_display_path_passthrough() {
        ...
        let tmp = std::env::temp_dir().join(format!("NETON-cdp-{}", std::process::id()));
    ...
    /// 仅在设置 NETON_FAKE_OPENAI_URL 环境变量时运行：
    ///   NETON_FAKE_OPENAI_URL=http://127.0.0.1:9901/v1 cargo test list_models_from_fake_openai
    #[tokio::test]
    async fn test_list_models_from_fake_openai() {
        let Ok(base) = std::env::var("NETON_FAKE_OPENAI_URL") else {
            eprintln!("跳过：未设置 NETON_FAKE_OPENAI_URL");
            return;
        };
