...
/// x-NETON-* 三件套为 NETONsign-v1 信道签名（手机端生成 → Worker 透传 → 本地验签）
    ...
    loop {
        let (base, rid, port, host, dev, bind) = {
            ...
            (
                ...
                // S = HMAC-SHA256(client_key, "NETON-worker-bind:{rid}")。Worker 收到后才能
                ...
                (!c.client_key.is_empty()).then(|| {
                    crate::security::hmac_sha256(
                        ...
                        format!("NETON-worker-bind:{rid_s}", rid_s = c.relay_id).as_bytes(),
        ...
        match poll_once(&client, &base, &rid, &dev, &bind).await {
            Ok(Some(req)) => {
                eprintln!("[NETON] relay tunnel <- {} {} (reqid {})", req.method, req.path, &req.rid[..8.min(req.rid.len())]);
                ...
                tokio::spawn(async move {
                    if let Err(e) = serve_and_answer(&client2, &ctx2, &base2, &rid2, req, port, &host2).await {
                        eprintln!("[NETON] relay serve failed: {e}");
                    }
                });
            }
            ...
            Err(e) => {
                // 网络抖动 / 服务器暂不可达：退避重试
                eprintln!("[NETON] relay poll error: {e}");
...
struct TunnelReq {
    ...
    stream: bool,
    /// 手机端真实公网 IP（Worker 从 cf-connecting-ip 附带）：回环时注入 x-NETON-client-ip，
    /// 本地 API 的 IP 黑名单 / 限速 / 并发上限按真实来源计数。手机端伪造不出该头
    /// （PASS_HEADERS 白名单不含它，只有本机 poller 会注入）
    client_ip: String,
    /// 许可签发时绑定的来源网段（Worker 从许可记录盖章，非现算）：回环时注入
    /// x-NETON-permit-pfx，本地 API 比对其与 client_ip 的网段一致性——不一致 = 中继
    /// 实现被替换/破坏，拒绝放行。与 client_ip 同为 poller 专属注入，手机端伪造不了
    permit_pfx: String,
}

/// 长轮询一次：Some=取到隧道请求；None=无待处理（204）。
/// dev 为设备指纹哈希（x-NETON-dev）：服务器做 rid↔设备绑定与单设备识别码上限。
/// bind 为信道握手密钥（x-NETON-bind，每识别码动态派生）：服务器收到后才启用
...
async fn poll_once(
    ...
    if !dev.is_empty() {
        rb = rb.header("x-NETON-dev", dev);
    }
    if !bind.is_empty() {
        rb = rb.header("x-NETON-bind", bind);
    }
    let resp = rb
...
/// 本机自加 x-NETON-via / x-NETON-rid 头：auth 中间件据此识别"经中继进入"并强制 NETONsign 验签
