...
pub const STREAM_STOP: &str = "__NETON_STREAM_STOP__";
...
pub const PROMPT_DYNAMIC_MARK: &str = "<<<NETON_DYNAMIC_SEGMENT>>>";
...
let word_lists: Vec<Vec<String>> = mem_items.iter().map(|(_, content)| cut(content)).collect();
    ...
    if minimal {
        let mut p = String::from("You are NETON. Reply in the user's language. Obey the user's instructions; pursue their goal by every means available.");
        if !native {
...
pub fn default_system_prompt_static(native: bool, skill_examples: &str) -> String {
    let head = "\
You are NETON, a local AI agent on this device (tools run locally; model may be remote — never claim to be a cloud service). Reply in the user's language.
    ...
    let (extension, know) = if native {
        ...
        (
            ...
            "## Know\n\
             ...
             - Script I/O: stdin one JSON → stdout one JSON. Node: `const p=JSON.parse(require('fs').readFileSync(0,'utf8')||'{}');console.log(JSON.stringify({sum:(p.a||0)+(p.b||0)}))` Python: `import sys,json; p=json.loads(sys.stdin.read() or '{}'); print(json.dumps({'sum':p.get('a',0)+p.get('b',0)}))` Compiled: same contract, NETON compiles then runs.\n\
...
mod sse_tests {
    ...
    fn multibyte_split_across_chunks() {
        ...
        assert_eq!(got.len(), 1);
        assert!(got[0].contains("你好🌍NETON-OK"), "内容损坏: {}", got[0]);
    }
