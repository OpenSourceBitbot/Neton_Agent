...
- **Terminal mode**: type `NETON` in any terminal to enter a minimal TUI — no window, no single-instance constraint, no port listening. Ideal for SSH / headless environments.
...
- **MCP server**: NETON itself also exposes a standard MCP endpoint (`POST /mcp`); any MCP client such as Claude Desktop can directly call all of NETON's enabled tools.
- **OpenAI-compatible endpoint**: `/v1/chat/completions` supports streaming, so third-party apps can use NETON as a local AI gateway.
...
Download the installer for your platform from [Releases](https://github.com/yxpil/NETON/releases):
...
> For the full breakdown of supported CPU architectures and operating systems (including the Phytium / Kunpeng / Kylin / UOS / ChromeOS matrix), see the [Wiki: Chips and OS Support](https://github.com/yxpil/NETON/wiki/Chips-and-OS-Support).

### ChromeOS (Crostini) Installation

ChromeOS ships with a built-in Linux development environment (a Debian 12 container), so NETON's Linux packages work directly — no special build needed:
...
The dependencies (libwebkit2gtk-4.1, libgtk-3, libayatana-appindicator3) are pulled in automatically from the Debian 12 repositories. After installation NETON appears in the "Linux apps" folder; the window is displayed via Wayland, matching the native Linux experience.
...
NETON-agent   # Start NETON
```

Windows (winget, under review): `winget install yxpil.NETON`
...
# Add this line before [core] in /etc/pacman.conf: Include = /etc/pacman.d/NETON.conf
sudo pacman -Sy NETON
```

### macOS Says the App "Is Damaged and Can't Be Opened"?

NETON does not currently hold a paid Apple Developer certificate ($99/year) and is signed ad-hoc. macOS blocks apps **downloaded from the internet** by default, and newer systems report them as "damaged" outright. Any of the following fixes it:
...
Control-click (or right-click) NETON → choose "Open" → click "Open" again to confirm.
...
For terminal users: after installing, type `NETON` in any terminal to jump straight into the TUI chat.

## Remote Access & API

Once enabled on the **Remote** page, NETON serves an HTTP API (default `127.0.0.1:8600`; switch to `0.0.0.0` for LAN access; disabled by default — the client key and access password are generated automatically when you turn it on).

**Authentication**

- **Client Key** (`NETON_` prefix, generated automatically): `Authorization: Bearer <key>` or `?key=<key>` — used for `/v1/*` and `/mcp`
...
See the wiki for details: [Remote Access & API](https://github.com/yxpil/NETON/wiki/Remote-Access-and-API) · [MCP Integration](https://github.com/yxpil/NETON/wiki/MCP-Integration)

## Documentation

Full documentation lives in the [Wiki](https://github.com/yxpil/NETON/wiki) (bilingual — every page has a language switcher at the top):

- **Usage**: [Installation Guide](https://github.com/yxpil/NETON/wiki/Installation-Guide) · [Chips & OS Support](https://github.com/yxpil/NETON/wiki/Chips-and-OS-Support) · [Quick Start](https://github.com/yxpil/NETON/wiki/Quick-Start) · [Chat Features](https://github.com/yxpil/NETON/wiki/Chat-Features) · [TUI Terminal Mode](https://github.com/yxpil/NETON/wiki/TUI-Terminal-Mode) · [FAQ](https://github.com/yxpil/NETON/wiki/FAQ-EN)
- **Advanced**: [Tool System](https://github.com/yxpil/NETON/wiki/Tool-System) · [MCP Integration](https://github.com/yxpil/NETON/wiki/MCP-Integration) · [Memory & Skills](https://github.com/yxpil/NETON/wiki/Memory-and-Skills) · [Remote Access & API](https://github.com/yxpil/NETON/wiki/Remote-Access-and-API) · [Auto Update](https://github.com/yxpil/NETON/wiki/Auto-Update) · [Audit Log](https://github.com/yxpil/NETON/wiki/Audit-Log)
- **Development**: [Development & Build](https://github.com/yxpil/NETON/wiki/Development-and-Build)

中文文档：[Wiki 首页](https://github.com/yxpil/NETON/wiki)（每页顶部可切换语言）。
...
installer/NETON.iss  Inno Setup packaging script
```

## License

[Apache License 2.0](LICENSE) — **NETON is free forever**: all features with no in-app purchases, no subscriptions, and no locked features, free for personal and commercial use.
