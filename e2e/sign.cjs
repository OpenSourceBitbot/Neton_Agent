// yxpil · NETON — NETONsign-v2 签名工具（与 src-tauri/src/security.rs 算法一致，E2E/真机验证用）
// canonical = "NETONsign-v2\n{rid}\n{ts}\n{nonce}\n{method}\n{path}\n{device_material}"
// device_material = 设备凭证的加盐 SHA256 前 16 hex（device.rs::sig_material 同口径：
//                   sha256("NETONdev-material:" + device_key) 前 16 hex）
// mac = HMAC-SHA256(key=client_key, canonical)；k = SHA256("NETONsign-v2:"+client_key)
// out[i] = rotl(mac[i], (i%7)+1) ^ k[i%16]；sign = hex(out)
const crypto = require("crypto");

function NETONsign(clientKey, deviceMaterial, rid, ts, nonce, method, path) {
  const canonical = `NETONsign-v2\n${rid}\n${ts}\n${nonce}\n${method}\n${path}\n${deviceMaterial}`;
  const mac = crypto.createHmac("sha256", clientKey).update(canonical).digest();
  const k = crypto.createHash("sha256").update(`NETONsign-v2:${clientKey}`).digest();
...
function deviceMaterial(deviceKey) {
  return crypto.createHash("sha256").update(`NETONdev-material:${deviceKey}`).digest().subarray(0, 8).toString("hex");
}
...
function signHeaders(clientKey, deviceMaterial, rid, method, path, opts = {}) {
  ...
  return {
    "x-NETON-sign": NETONsign(clientKey, deviceMaterial, rid, ts, nonce, method, path),
    "x-NETON-ts": String(ts),
    "x-NETON-nonce": nonce,
  };
}
...
/** 信道握手密钥：S = HMAC-SHA256(key=client_key, msg="NETON-worker-bind:{rid}")（64 hex）。
 *  NETON 端每次 poll 经 X-NETON-Bind 下发给服务器；手机端从二维码密文内的 client_key 自行派生 */
function workerBind(clientKey, rid) {
  return crypto.createHmac("sha256", clientKey).update(`NETON-worker-bind:${rid}`).digest("hex");
}
...
/** 完整握手（生产 osbt.space）：取挑战 → 算应答，返回许可请求头（x-NETON-challenge / x-NETON-proof）。
...
async function handshakeHeaders(rid, bindHex) {
  ...
  return { "x-NETON-challenge": c, "x-NETON-proof": proofOf(bindHex, c) };
}

module.exports = { NETONsign, deviceMaterial, signHeaders, workerBind, proofOf, handshakeHeaders };
