// 简约 TUI E2E：`NETON tui` 终端模式功能完整性 + 与桌面端同数据目录并行运行无冲突
// 覆盖：/help 对话 工具调用 会话管理 记忆 install-cli 桌面端/TUI 同跑
// 用法：mock-ai(9901) 就绪后：node e2e/tui.cjs [NETON 二进制路径]
...
const BIN = path.resolve(process.argv[2] || path.join(__dirname, "../src-tauri/target/release/NETON"));
...
// 运行中同路径 NETON 实例的 PID 列表（避免日常实例的单实例保护干扰测试）
...
function launchTui(dir, extraEnv = {}, opts = {}) {
  const proc = spawn(BIN, ["tui"], {
    ...
    env: { ...process.env, NO_AT_BRIDGE: "1", NETON_DATA_DIR: dir, ...extraEnv },
  ...
  // 等待 REPL 就绪（首个 "NETON> " 提示符）：xvfb/无 GPU 环境 webkit 栈初始化可达数十秒，
  // 就绪前发送的指令虽会被缓冲，但 AI 请求会挤占后续断言的轮询窗口（T17-ARRAY/T19a 假失败根因）
  const ready = (timeout = 90000) =>
    new Promise((resolve) => {
      const t0 = Date.now();
      const poll = () => {
        if (out.includes("NETON> ")) return resolve(true);
...
async function main() {
  ...
  // NETON 默认原生 function-calling 模式（compat_mode=false）不会执行正文里的文本调用，
  // 工具链路场景必须显式走文本约定协议
  const DIR = fs.mkdtempSync(path.join(os.tmpdir(), "NETON-tui-e2e-"));
  fs.writeFileSync(
    path.join(DIR, "config.json"),
    // Config 的 remote_enabled/host/port/client_key/revision 无 serde(default)，
    // 缺任一字段整份解析失败回退默认值（compat_mode 静默失效）——必须写全
    JSON.stringify({ compat_mode: true, remote_enabled: false, host: "127.0.0.1", port: 8611, client_key: "NETON_e2e_tui_key", revision: 1 })
  );
  fs.writeFileSync(
  ...
  // ── T6 /install-cli：macOS/Linux 符号链接 / Windows NETON.cmd 启动器 ──
  {
    const CLI_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "NETON-tui-cli-"));
    const tui = launchTui(DIR, { NETON_CLI_DIR: CLI_DIR });
    await tui.ready();
    ...
    if (process.platform === "win32") {
      // Windows 写 NETON.cmd 启动脚本（内含 exe 路径与 tui 参数）
      const launcher = path.join(CLI_DIR, "NETON.cmd");
      try {
        const content = fs.readFileSync(launcher, "utf8");
        ok = /tui/.test(content) && content.toLowerCase().includes("NETON");
        detail = content.trim().split(/\r?\n/).pop() || "";
      } catch (e) { detail = e.message; }
    } else {
      const link = path.join(CLI_DIR, "NETON");
      try {
  ...
  // ── T9 空行与纯空白被忽略；T16 /q 变体退出 ──
  {
    ...
    const code = await tui.waitExit();
    // 空白行不应产生"错误"输出
    const ok = code === 0 && !tui.out.split("NETON TUI")[1]?.includes("错误");
  ...
  {
    const BAD = fs.mkdtempSync(path.join(os.tmpdir(), "NETON-tui-bad-"));
  ...
  {
    const DEAD = fs.mkdtempSync(path.join(os.tmpdir(), "NETON-tui-dead-"));
    fs.writeFileSync(
  ...
  // ── T19 流式边界：多字节跨 chunk / 200 小块 / 流中输入模拟 / 立即 500 ──
  {
    ...
    for (let i = 0; i < 60 && !tui.out.includes("NETON-STREAM-OK"); i++) await sleep(500);
    await sleep(300);
    const mbOk = tui.out.includes("你好🌍NETON-STREAM-OK") && !tui.out.includes("�");
  ...
  // ── T7 桌面端 + TUI 同数据目录并行：互不干扰 ──
  {
    ...
    fs.writeFileSync(
      path.join(DIR, "config.json"),
      JSON.stringify({ remote_enabled: true, host: "127.0.0.1", port: PORT, client_key: "NETON_e2e_tui_key", password_enabled: false, revision: 1 })
    );
    const desktop = spawn(BIN, [], {
      env: { ...process.env, NETON_DATA_DIR: DIR, NETON_HEADLESS: "1" },
  ...
  {
    const CDIR = fs.mkdtempSync(path.join(os.tmpdir(), "NETON-tui-cache-"));
    fs.writeFileSync(
      path.join(CDIR, "config.json"),
      JSON.stringify({ compat_mode: false, auto_drive: false, remote_enabled: false, host: "127.0.0.1", port: 8612, client_key: "NETON_e2e_cache_key", revision: 1 })
    );
    fs.writeFileSync(
    ...
    const dump = path.join(os.tmpdir(), "NETON-e2e-cache-dump.jsonl");
  ...
  {
    const SDIR = fs.mkdtempSync(path.join(os.tmpdir(), "NETON-tui-silent-"));
    fs.writeFileSync(
      path.join(SDIR, "config.json"),
      JSON.stringify({ compat_mode: false, auto_drive: false, remote_enabled: false, host: "127.0.0.1", port: 8613, client_key: "NETON_e2e_silent_key", revision: 1 })
    );
    fs.writeFileSync(
  ...
  {
    const WS = fs.mkdtempSync(path.join(os.tmpdir(), "NETON-ws-root-"));
    const CDIR = fs.mkdtempSync(path.join(os.tmpdir(), "NETON-tui-ws-"));
    fs.writeFileSync(path.join(CDIR, "config.json"), JSON.stringify({ compat_mode: true, remote_enabled: false, host: "127.0.0.1", port: 8615, client_key: "NETON_e2e_ws_key", revision: 1 }));
    ...
    const escapeFile = path.join(os.tmpdir(), `NETON-escape-${Date.now()}.txt`);
    ...
    await waitFor(tui, "E2E-FINAL-OK");
    await sleep(300);
    const relFile = path.join(WS, "NETON-ws-probe.txt");
  ...
  {
    const NDIR = fs.mkdtempSync(path.join(os.tmpdir(), "NETON-tui-cmds-"));
    fs.writeFileSync(path.join(NDIR, "config.json"), JSON.stringify({ compat_mode: true, remote_enabled: false, host: "127.0.0.1", port: 8616, client_key: "NETON_e2e_cmd_key", revision: 1 }));
