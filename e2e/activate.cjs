...
// 目录判定：NETON_DATA_DIR 优先（E2E 隔离目录与被测实例一致），
// 否则 Windows 用 %APPDATA%，macOS/Linux 用家目录固定路径（与 NETON 数据目录一致）
const fs = require("fs");
const os = require("os");
const path = require("path");
const envDir = process.env.NETON_DATA_DIR;
const dir = envDir
  || (process.platform === "win32"
    ? path.join(process.env.APPDATA, "com.NETON.hub")
    : path.join(os.homedir(), ".local", "share", "com.NETON.hub"));
const macDir = path.join(os.homedir(), "Library", "Application Support", "com.NETON.hub");
