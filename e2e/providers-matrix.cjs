...
// 通过 /api/debug/config 的 active_provider 热切换上游；NETON 侧统一走标准原生协议（compat_mode=false）。
// 前置：实例已用隔离目录启动（NETON_DATA_DIR=/tmp/NETON-e2e…），且已 node e2e/activate.cjs 注册三家 mock 提供方。
// 用法：E2E_KEY=… E2E_PASSWORD=… node e2e/providers-matrix.cjs
const http = require("http");
const BASE = "127.0.0.1", PORT = 8600;
const KEY = process.env.E2E_KEY || "NETON_e2e_client_key_0000000000000000";
