// yxpil · NETON — 生产 Worker 真机全链路验证（osbt.space ↔ 本地 NETON）
// 含连接许可（permit）协议：动态握手密钥 + 挑战-应答，App 正确沟通才允许连接
// 用法：node e2e/prod_check.cjs [NETONPort=8600] [NETONKey] [NETONPwd]
const http = require("http");
const https = require("https");
const { signHeaders, deviceMaterial, workerBind, proofOf } = require("./sign.cjs");
const cfg = require("os").homedir() + "/Library/Application Support/com.NETON.hub/config.json";
    ...
    const req = https.request(
      ...
      (res) => {
        ...
        res.on("end", () => resolve({ code: res.statusCode, body: b, ct: res.headers["content-type"],
          chunks: chunks.length, permit: res.headers["x-NETON-permit"] || "" }));
      });
    req.on("error", reject);
...
const tunnelReq = async (rid, path, method, headers, body, raw) => {
  const h = { ...headers };
  if (permitCache.has(rid)) h["x-NETON-permit"] = permitCache.get(rid);
  ...
  if (r.code === 403 && /permit required/i.test(r.body)) {
    ...
    const retry = { ...headers, "x-NETON-challenge": c, "x-NETON-proof": proofOf(workerBind(KEY, rid), c) };
...
// 等 NETON 至少完成一次 poll（bind 已送达 Worker、许可门槛生效），避免竞态误判
...
(async () => {
  // 0) 设备签名材料（NETONsign-v2）：从被测实例 config.json 读 device_key 派生
  ...
  // 2) 等 NETON 首次 poll 把握手密钥送达 Worker（此后许可门槛生效）
  ...
  const ka = await new Promise((resolve, reject) => {
    const q = https.get({ host: HOST, path: `/relay/ka/${rid}`, timeout: 15000, headers: {
      ...signHeaders(KEY, MAT, rid, "GET", "/relay/ka"), "x-NETON-permit": permitCache.get(rid) || "",
    } }, (r) => { let b = ""; r.on("data", (c) => (b += c)); r.on("end", () => resolve({ code: r.statusCode, body: b, permit: r.headers["x-NETON-permit"] || "" })); });
