...
// 用法：node e2e/remote-default.cjs [NETON 二进制路径]
...
const BIN = process.argv[2] || path.join(__dirname, "../src-tauri/target/debug/NETON");
const results = [];
...
// 用隔离数据目录启动一个 NETON 实例（无头模式，不弹窗），返回 { proc, dir }
// stderr 保留尾部用于失败诊断（如单实例冲突、启动异常）
function launchNETON(dir) {
...
(async () => {
  // ── 前置：停掉运行中的 NETON 实例，避免单实例保护顶掉测试实例 ──
  const conflicts = findConflicts();
  if (conflicts.length) {
    console.log(`[setup] 停止 ${conflicts.length} 个运行中的 NETON 实例（避免单实例冲突）: ${conflicts.join(", ")}`);
  ...
  {
    const { proc, dir } = launchNETON(path.join(os.tmpdir(), `NETON-remote-off-${Date.now()}`));
    try {
      let cfgFile = {};
      for (let i = 0; i < 30; i++) {
        await sleep(500);
  ...
  {
    const KEY = "NETON_e2e_remote_default_test_key";
    const port = 8765;
    const dir = path.join(os.tmpdir(), `NETON-remote-on-${Date.now()}`);
    const { proc } = launchNETON(dir);
    try {
      await sleep(1000); // 等 NETON 先生成完整默认配置
      fs.writeFileSync(
      ...
      // NETON 启动时只读一次配置，改配置需重启才生效
      proc.kill();
      await sleep(1000);
      const { proc: again } = launchNETON(dir);
      try {
        let listening = false;
        for (let i = 0; i < 20; i++) {
          await sleep(500);
        ...
        if (listening) {
          ...
          const bad = await getStatus(port, "/api/tools", { Authorization: "Bearer NETON_wrong", "X-Access-Password": "12345678" });
