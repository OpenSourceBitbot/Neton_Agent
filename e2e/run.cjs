// E2E 驱动脚本：通过 NETON 远程 API 驱动完整 AI 链路（mock 上游），逐场景断言
// 默认从 NETON 数据目录 config.json 读取 client_key / access_password，可用 E2E_PORT / E2E_KEY / E2E_PASSWORD 覆盖
...
let cfg = {};
try {
  cfg = JSON.parse(fs.readFileSync(os.homedir() + "/Library/Application Support/com.NETON.hub/config.json", "utf8"));
} catch {}
...
// 设备签名材料（NETONsign-v2）：读实例 config.json 的 device_key（启动自动注册），
...
async function devMat() {
  ...
  while (Date.now() < deadline) {
    try {
      ...
      if (typeof c.device_key === "string" && c.device_key.startsWith("NETONdev_")) {
...
const AI_CFG = os.homedir() + "/Library/Application Support/com.NETON.hub/ai_config.json";
const AI_CFG_DISK_BACKUP = "/tmp/NETON-ai-config-backup.json";
...
(async () => {
  ...
  try {
    ...
    if (process.env.E2E_ALLOW_REAL_DATA !== "1" && !/e2e/i.test(dd)) {
      ...
      console.error("请用隔离目录启动实例，例如: NETON_DATA_DIR=/tmp/NETON-e2e ... ；确需对真实目录跑请设 E2E_ALLOW_REAL_DATA=1");
      process.exit(1);
    }
  ...
  // NETON 现在会把目标推进到全部待办完成并自动归档，最终确认语可能是 plan 的 E2E-FINAL-OK，
  ...
  // T18 看图工具：view_image 读取本地图片 → 图片注入下一轮请求（mock 确认收到 image_url）→ 记录脱敏无 base64
  {
    ...
    try {
      delView();
      // NETON 进程工作目录为 src-tauri，相对路径 ./ 解析在那里；两处都放一份兜底
  ...
  // T20 远程访问鉴权：缺失密码 / 错误密码 / 错误 Key 均拒绝（负例在路由前被 auth 中间件拦截）
  {
    try {
      ...
      const badKey = await getStatus("/api/tools", { Authorization: "Bearer NETON_wrongkey_e2e", "X-Access-Password": PASSWORD });
  ...
  // NETON 在回复尾部附 [repetition-guard] 标记（同时暂停该会话 auto-drive）
  try {
  ...
  // 协议（poll/req/answer），NETON 中继循环主动出站长轮询，隧道请求回环打本地 API 后原样回传
  const RPORT = Number(process.env.FAKE_RELAY_PORT) || 9802;
  ...
  // 握手密钥（X-NETON-Bind）随 NETON poll 下发就位后，隧道请求必须持许可或现场握手；
  ...
  const tunnelReq = async (rid, path, method, headers, body) => {
    ...
    if (permitCache.has(rid)) {
      authHeaders["x-NETON-permit"] = permitCache.get(rid);
    } else {
      const c = await challenge(rid);
      Object.assign(authHeaders, { "x-NETON-challenge": c, "x-NETON-proof": proofOf(workerBind(KEY, rid), c) });
    }
    let r = await rawReq(rid, path, method, authHeaders, b);
    if (r.headers?.["x-NETON-permit"]) permitCache.set(rid, r.headers["x-NETON-permit"]);
    if (r.code === 403 && /permit required/.test(r.body)) {
      permitCache.delete(rid);
      const c = await challenge(rid);
      const retry = { ...headers, ...ct, "x-NETON-challenge": c, "x-NETON-proof": proofOf(workerBind(KEY, rid), c) };
  ...
  // 设备凭证就绪：NETONsign-v2 材料依赖 device_key（启动自动注册），后续全部隧道用例共用
  ...
  try {
    ...
    // 4) 隧道 GET：/api/health 经中继转发回本地 API（真鉴权头原样透传 + NETONsign-v2 签名）
  ...
  try {
    ...
    //    nonce 每轮唯一（含 RUN）：NETON 端 nonce 缓存窗口 = 签名时间窗（120s），
    ...
    //    开源客户端的防滥用只能靠服务器，这正是 NETONsign 站点层存在的意义）
  ...
  // ── T46 二维码加密块：enc=NETON1: 密文（不含明文 key/rid）+ alg 标识（NETONsign-v2）──
  try {
    const p46 = JSON.parse((await callGet("/api/qr")).body);
    const encOk = typeof p46.enc === "string" && p46.enc.startsWith("NETON1:") && p46.enc.length > 64 &&
      !p46.enc.includes(KEY) && !p46.enc.includes(p46.rid);
    const algOk = p46.alg === "NETONsign-v2";
  ...
  // ── T48 设备凭证：自动注册落盘（NETONdev_* + 时间戳 + 指纹哈希）且幂等稳定 ──
  try {
    const dd = String(JSON.parse((await callGet("/api/debug/state")).body).data_dir || "");
    const read = () => JSON.parse(fs.readFileSync(dd + "/config.json", "utf8"));
    const c1 = read();
    const keyOk = /^NETONdev_[0-9a-f]{32}$/.test(c1.device_key || "");
  ...
  try {
    await call("/api/debug/config", { cloud_relay_url: `http://127.0.0.1:${RPORT}` });
    // NETON poller 已用本机 device_fp_hash poll 过 → 绑定生效；status 应返回绑定设备前 8 hex
    ...
    // 绑定需要 NETON poller 至少完成一次 poll（配置热生效 ~3s），轮询等待最多 12s
    ...
    const rawPoll = (dev) => new Promise((resolve, reject) => {
      ...
      if (dev) headers["x-NETON-dev"] = dev;
  ...
  // 握手密钥 S = HMAC(client_key, "NETON-worker-bind:{rid}") 随 NETON poll（X-NETON-Bind）
  ...
  try {
    ...
    // a) 等 NETON poller 送达握手密钥（许可强制生效开关）：无许可请求翻转为 403
    ...
    for (;;) {
      // 每次尝试重新签名（nonce 一次性，复用会被 NETON 判 replay 干扰判断）；
      // 只认中继侧 "permit required" 403（许可闸门生效信号），NETON 侧 403 继续等
    ...
    const badProof = await rawReq(rid52, "/api/health", "GET", {
      Authorization: `Bearer ${KEY}`, "X-Access-Password": PASSWORD,
      ...signHeaders(KEY, MAT, rid52, "GET", "/api/health"),
      "x-NETON-challenge": cBad, "x-NETON-proof": proofOf("ab".repeat(32), cBad),
    });
    // c) 正确握手（S 派生应答）→ 200 + 许可签发（x-NETON-permit 响应头带回）
    const cOk = await challenge(rid52);
    const handshake = await rawReq(rid52, "/api/health", "GET", {
      Authorization: `Bearer ${KEY}`, "X-Access-Password": PASSWORD,
      ...signHeaders(KEY, MAT, rid52, "GET", "/api/health"),
      "x-NETON-challenge": cOk, "x-NETON-proof": proofOf(BIND, cOk),
    });
    const permitTok = handshake.headers?.["x-NETON-permit"] || "";
    const permitOk = handshake.code === 200 && /^[0-9a-f]{32}$/.test(permitTok);
    // d) 持许可后续请求直接放行（无需再握手）
    const withPermit = await rawReq(rid52, "/api/health", "GET", {
      Authorization: `Bearer ${KEY}`, "X-Access-Password": PASSWORD,
      ...signHeaders(KEY, MAT, rid52, "GET", "/api/health"),
      "x-NETON-permit": permitTok,
    });
    ...
    const kaOk = await kaReq(rid52, { Authorization: `Bearer ${KEY}`, "X-Access-Password": PASSWORD,
      ...signHeaders(KEY, MAT, rid52, "GET", "/relay/ka"), "x-NETON-permit": permitTok });
    const kaRenew = /^[0-9a-f]{32}$/.test(kaOk.headers?.["x-NETON-permit"] || "");
    let kaOnline = false;
    ...
    // f) 来源一致性（NETON 端 IP 比对闸门，确保真实性才放行）：先从新网段 203.0.113.9
    //    正常握手取许可（Worker 端许可本就绑定网段），再模拟"中继被替换 / 盖章被破坏"
    //    ——x-fake-stamp-pfx 强制盖章为不一致网段 → NETON 端比对真实 IP 网段不符 → 403；
    ...
    const mintF = await rawReq(rid52, "/api/health", "GET", {
      Authorization: `Bearer ${KEY}`, "X-Access-Password": PASSWORD,
      ...signHeaders(KEY, MAT, rid52, "GET", "/api/health"),
      "x-NETON-challenge": chalF, "x-NETON-proof": proofOf(BIND, chalF), "x-fake-client-ip": ipF,
    });
    const permitF = mintF.headers?.["x-NETON-permit"] || "";
    const stampBad = await rawReq(rid52, "/api/health", "GET", {
      Authorization: `Bearer ${KEY}`, "X-Access-Password": PASSWORD,
      ...signHeaders(KEY, MAT, rid52, "GET", "/api/health"),
      "x-NETON-permit": permitF, "x-fake-client-ip": ipF, "x-fake-stamp-pfx": "198.51.100",
    });
    const stampOk = await rawReq(rid52, "/api/health", "GET", {
      Authorization: `Bearer ${KEY}`, "X-Access-Password": PASSWORD,
      ...signHeaders(KEY, MAT, rid52, "GET", "/api/health"),
      "x-NETON-permit": permitF, "x-fake-client-ip": ipF,
    });
  ...
  // ── T54 多用户不串线：多手机共用一个 NETON + 跨网段/跨 rid/密钥轮换隔离 ──
  try {
    await call("/api/debug/config", { cloud_relay_url: `http://127.0.0.1:${RPORT}` });
    ...
    // 模拟第二位用户（另一台 NETON）：独立 rid + 独立握手密钥（其 poller 即本人）
    ...
    const pollRid = (rid, bindHex) => new Promise((resolve, reject) => {
      const q = http.request({ host: BASE, port: RPORT, path: `/relay/poll/${rid}`, method: "POST",
        headers: { "x-NETON-dev": "1234567890abcdef", "x-NETON-bind": bindHex, "content-type": "application/json" }, timeout: 10000, agent },
        (r) => { let b = ""; r.on("data", (c) => (b += c)); r.on("end", () => resolve(b)); });
      q.on("error", reject); q.end("{}");
    });
    await pollRid(rid54b, "cd".repeat(32));

    // 手机端模拟器：ip 模拟真实公网来源（fake_relay 经 x-fake-client-ip 透传给 NETON 计数）
    const phone = (rid, ip, bindHex) => ({
      handshake: async () => {
        ...
        const r = await rawReq(rid, "/api/health", "GET", {
          ...
          "x-NETON-challenge": c, "x-NETON-proof": proofOf(bindHex, c) });
        return { code: r.code, permit: r.headers?.["x-NETON-permit"] || "" };
      },
      req: (path, method, permit, body) => {
        ...
        return rawReq(rid, path, method, {
          ...
          ...signHeaders(KEY, MAT, rid, method, path), "x-NETON-permit": permit }, b);
      },
    });
    ...
    const acrossChal = await rawReq(rid54b, "/api/health", "GET", {
      ...
      "x-NETON-challenge": cX, "x-NETON-proof": proofOf(BIND54, cX) });
    const acrossPermit = await rawReq(rid54b, "/api/health", "GET", {
      Authorization: `Bearer ${KEY}`, "X-Access-Password": PASSWORD, "x-fake-client-ip": "203.0.113.10",
      ...signHeaders(KEY, MAT, rid54b, "GET", "/api/health"),
      "x-NETON-permit": hsA.permit });

    // e) 密钥轮换即吊销：rid54b 换 bind 后旧许可立即失效
    // rid54b 无真实 NETON poller —— 以"第二台 NETON 模拟器"持续 poll 并代答 200，
    ...
    const answerReq = (t) => new Promise((resolve, reject) => {
      const data = JSON.stringify({ rid: t.rid, s: 200, h: { "content-type": "application/json" },
        b: Buffer.from(JSON.stringify({ ok: true, via: "second-NETON" })).toString("base64") });
  ...
  try {
    ...
    const hsB = await rawReq(rid55, "/api/health", "GET", {
      ...
      ...signHeaders(KEY, MAT, rid55, "GET", "/api/health"),
      "x-NETON-challenge": cB, "x-NETON-proof": proofOf(BIND55, cB) });
    const tB = Date.now();
    const bSmall = await rawReq(rid55, "/api/health", "GET", {
      Authorization: `Bearer ${KEY}`, "X-Access-Password": PASSWORD, "x-fake-client-ip": "198.51.100.99",
      ...signHeaders(KEY, MAT, rid55, "GET", "/api/health"),
      "x-NETON-permit": hsB.headers?.["x-NETON-permit"] || "" });
  ...
  try {
    ...
    // a) chat-only：全凭据 + 合法许可打非聊天路径 → NETON 403（签名与许可再合法也只有对话可用）
    ...
    for (let i = 101; i <= 110 && churn429 === 0; i++) {
      ...
      const r = await rawReq(rid56, "/api/health", "GET", {
        ...
        ...signHeaders(KEY, MAT, rid56, "GET", "/api/health"),
        "x-NETON-challenge": c, "x-NETON-proof": proofOf(BIND56, c) });
      if (r.code === 200 && r.headers?.["x-NETON-permit"]) { churnOk++; lastGood = r.headers["x-NETON-permit"]; lastGoodIp = `198.51.${i}.1`; }
    ...
    if (lastGood) {
      still = await rawReq(rid56, "/api/health", "GET", {
        ...
        ...signHeaders(KEY, MAT, rid56, "GET", "/api/health"), "x-NETON-permit": lastGood });
    }
  ...
  try {
    ...
    try {
      ...
      const hs1 = await rawReq(rid57, "/api/health", "GET", {
        ...
        "x-NETON-challenge": c1, "x-NETON-proof": proofOf(BIND57, c1) }, null, 9805);
      const P1 = hs1.headers?.["x-NETON-permit"] || "";
      const inEra = await rawReq(rid57, "/api/health", "GET", {
        Authorization: `Bearer ${KEY}`, "X-Access-Password": PASSWORD,
        ...signHeaders(KEY, MAT, rid57, "GET", "/api/health"), "x-NETON-permit": P1 }, null, 9805);
      ...
      const hs2 = await rawReq(rid57, "/api/health", "GET", {
        Authorization: `Bearer ${KEY}`, "X-Access-Password": PASSWORD,
        ...signHeaders(KEY, MAT, rid57, "GET", "/api/health"),
        "x-NETON-challenge": c2, "x-NETON-proof": proofOf(BIND57, c2) }, null, 9805);
      const P2 = hs2.headers?.["x-NETON-permit"] || "";
      const healed = await rawReq(rid57, "/api/health", "GET", {
        Authorization: `Bearer ${KEY}`, "X-Access-Password": PASSWORD,
        ...signHeaders(KEY, MAT, rid57, "GET", "/api/health"), "x-NETON-permit": P2 }, null, 9805);
  ...
  try {
    ...
    // NETON 的业务错误通过 HTTP body 返回（code=200 但 body 含 error 字段），
