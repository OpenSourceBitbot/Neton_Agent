// yxpil · NETON — 手机远程访问多客户端会话隔离测试（防对话串线）
...
// 用法：node e2e/mobile-isolation.cjs [NETON二进制路径] [端口=18610]
...
(async () => {
  if (!fs.existsSync(BIN)) {
    console.error(`NETON 二进制不存在：${BIN}`);
    process.exit(2);
  }
  ...
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "NETON-isolation-"));
  ...
  const NETON = spawn(BIN, [], {
    env: { ...process.env, NETON_DATA_DIR: dir, NETON_HEADLESS: "1", NETON_NO_GUARDIAN: "1" },
    stdio: ["ignore", "ignore", "pipe"],
    detached: false,
  });
  let errTail = "";
  NETON.stderr.on("data", (d) => { errTail = (errTail + d.toString()).slice(-600); });
  NETON.on("exit", (c) => { if (c !== 0) console.error(`NETON 提前退出 code=${c} stderr: ${errTail}`); });

  let up = false;
  const dl = Date.now() + 30000;
  while (Date.now() < dl) {
    if (NETON.exitCode !== null) break;
    if (await portOpen(PORT)) { up = true; break; }
    await sleep(400);
  }
  if (!up) {
    console.error(`NETON 30s 未监听 ${PORT}，stderr: ${errTail}`);
    try { NETON.kill("SIGKILL"); } catch {}
    process.exit(2);
  }
