// 自动更新全流程 e2e（隔离数据目录 + NETON_FAKE_UPDATE_URL 注入 mock 更新源）：
// 1) 检测：latest.json → has_update / 无更新两个分支
// 2) 下载：/api/update/download 落盘 upgrade/、幂等缓存（重复调用不重复下载）
// 3) 状态：/api/update/status → downloaded
// 4) 启动自动下载：NETON 启动后台任务 6 秒后静默拉包（不发任何 API 请求也应完成）
// 用法：node e2e/update.cjs [NETON 二进制路径]
...
const API_PORT = 8766;
const KEY = "NETON_e2e_update_test_key";
...
function launchNETON(dir, fakeUrl) {
  ...
  const proc = spawn(BIN, [], {
    env: { ...process.env, NETON_DATA_DIR: dir, NETON_HEADLESS: "1", NETON_FAKE_UPDATE_URL: fakeUrl },
...
(async () => {
  ...
  // ── U1-U4：有更新的全流程 ──
  {
    const dir = path.join(os.tmpdir(), `NETON-update-${Date.now()}`);
    const proc = launchNETON(dir, `http://127.0.0.1:${MOCK_PORT}/latest.json`);
    try {
      let listening = false;
  ...
  // ── U5：无更新分支（旧版本源）──
  {
    const dir = path.join(os.tmpdir(), `NETON-update-none-${Date.now()}`);
    const proc = launchNETON(dir, `http://127.0.0.1:${MOCK_PORT}/old/latest.json`);
    try {
      let listening = false;
      for (let i = 0; i < 24 && !listening; i++) { await sleep(500); listening = await portOpen(API_PORT); }
  ...
  {
    const dir = path.join(os.tmpdir(), `NETON-update-auto-${Date.now()}`);
    const proc = launchNETON(dir, `http://127.0.0.1:${MOCK_PORT}/latest.json`);
    try {
      let st = null;
