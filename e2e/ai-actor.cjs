// AI Actor E2E：本脚本扮演「AI 模型」（OpenAI 兼容上游），驱动 NETON 的真实工具执行链路。
// 与 mock-ai 的区别：mock 是被动应答，actor 会对 NETON 回传的工具结果做【断言】——
...
const BIN = process.argv[2] || "src-tauri/target/debug/NETON";
const UP_PORT = 9905; // 扮演 AI 的上游端口
const APP_PORT = 8607; // 被测 NETON 实例端口
const KEY = "NETON_actor_client_key_000000000000";
...
const TMP_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "NETON-actor-work-"));
...
(async () => {
  ...
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "NETON-actor-"));
  fs.writeFileSync(
  ...
  if (!(await waitHealth())) {
    console.error(`AI-ACTOR FAIL: NETON 未就绪。stderr:\n${errTail}`);
    await cleanup();
    process.exit(1);
  }
