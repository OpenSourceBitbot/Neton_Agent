// 长文本 + 压力 E2E：走 NETON /api/chat 全链路（mock 上游），覆盖流中断与 max_tokens 截断
// 用法：mock-ai(9901) 与 NETON（隔离数据目录）就绪后：node e2e/stress.cjs
...
// 流式调用 NETON 的 OpenAI 兼容端点（触发上游流式链路），返回 SSE 原文
...
// ── F1 流中断：上游不发 [DONE] 直接断开 → NETON 自动回退非流式重试，保证完整答复+干净收尾 ──
{
...
// ── F2 max_tokens 截断：finish_reason=length → NETON 自动补发「继续」接续后半段，
