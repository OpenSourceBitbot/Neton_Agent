// E2E 模拟上游 AI（OpenAI 兼容）：模仿 NETON 的上游提供方，驱动全功能工具调用测试
...
// NETON 不把工具反馈持久化进下一轮历史，后续自动推进请求里拿不到 goal_id，
...
function toolResultCount(messages) {
  // 文本协议：user 消息以「Tool result(s)」开头（NETON 反馈前缀）；原生 function calling：role="tool" 消息
...
const CACHE_DUMP = path.join(os.tmpdir(), "NETON-e2e-cache-dump.jsonl");
...
function handleClaude(res, parsed) {
  ...
  // 端点形态：拒绝 tools 参数（标准协议带 tools 来就 400 + 含 tool 的错误体 → NETON 分类
  ...
  if (all.includes("E2E-CACHE-SILENT")) {
    ...
    // 沉淀型回合：只发一次 tool_use，NETON 不应回灌触发第二轮请求
...
const server = http.createServer((req, res) => {
  ...
  req.on("end", () => {
    ...
    // NETON 在 tool_loop_max 轮后拒绝执行并附 [tool-loop-guard] 熔断标记，不再询问 mock
    ...
    // NETON 应在回复尾部附 [repetition-guard] 熔断标记
    ...
    // ── 用户轮：按场景标记返回工具调用（含 NETON 文本协议的各种变体） ──
    ...
    if (last.startsWith("继续（自动推进）")) {
      ...
      // NETON 在待办全部完成时会先自动把目标置为 achieved，再发这条「所有待办均已完成」收尾消息——
    ...
    // 压测-上行完整性：回显收到的最后一条用户消息的统计（验证 NETON 完整转发大文本）
    ...
    // 压测-流中断：发两块内容后直接断开（不发 [DONE]），验证 NETON 不再把半截当完整回复
    ...
    // 压测-max_tokens 截断：finish_reason=length → NETON 应自动补发「继续」，
    ...
    // ── 模糊格式识别：不严格遵循 OpenAI 协议的变体响应（NETON 应动态识别而非报错） ──
    ...
    // 非 2xx 原生请求会让 NETON 静默降级到文本流式协议（read_json_native 分类 Unsupported），
    ...
    // 思考过程：先 reasoning_content 增量再正文（DeepSeek R1 风格），验证 NETON 聚合落库与 SSE 转发
    ...
    if (last.includes("E2E-WS-WRITE"))
      return respond(res, '[{"tool":"write_file","params":{"path":"NETON-ws-probe.txt","content":"ws-ok"}}]', sse);
    if (last.includes("E2E-WS-ESCAPE|")) {
    ...
    // 验证 NETON 按 id 分槽聚合（修复前全部并入槽 0 → name/args 交错成垃圾）
    ...
    // NETON 应识别为 Unsupported 并给出「兼容模式」指引
    ...
    if (last.includes("E2E-CMD-SEND"))
      return respond(
        res,
        '生成一个交付文件：\n[{"tool":"write_file","params":{"path":"./.e2e-send.txt","content":"hello from NETON e2e"}}]',
        sse
      );
    ...
    // 看图场景：让 NETON 调 view_image，第二轮请求应带上注入的图片（imgs>0 分支回 IMAGE-SEEN）
    ...
    // 压缩历史场景：让 NETON 调 compact_history，用摘要替换全部历史
