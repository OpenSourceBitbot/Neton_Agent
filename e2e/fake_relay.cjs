...
// 信道防护（NETONsign-v2，与 Worker 一致）：/relay/req 必须携带合法格式的
//   X-NETON-Sign / X-NETON-Ts / X-NETON-Nonce，缺失/格式非法 403 并计入坏签名；
//   单 IP 窗口内坏签名达 12 次 → 封禁 30 分钟（密码学验证在 NETON 端）
// 流式（与 Worker 一致）：请求体含 "stream":true 或 Accept: text/event-stream →
//   TransformStream 增量响应，NETON 逐块 answer
...
const server = http.createServer((req, res) => {
  ...
  req.on("end", () => {
    ...
    if (action === "poll" && req.method === "POST") {
      // rid↔设备绑定 + 单设备识别码上限（与 Worker 一致；NETON 端 poll 携带 X-NETON-Dev）
      const dev = req.headers["x-NETON-dev"] || "";
      if (!DEV_RE.test(dev)) return json(res, 403, { error: "device fingerprint required (x-NETON-dev, NETONsign-v2 client)" });
      ...
      // 信道握手密钥（X-NETON-Bind）：就位后许可强制生效（与 Worker /do/poll 同语义）；
      // 密钥轮换（bind 变更）即吊销旧许可与挑战（与 Worker 同语义）
      const bind = req.headers["x-NETON-bind"] || "";
    ...
    if (action === "answer" && req.method === "POST") {
      ...
      if (w.permit) aHeaders["x-NETON-permit"] = w.permit; // 新签发/续期的许可随响应头带回
      w.res.writeHead(a.s || 502, aHeaders);
      w.res.end(Buffer.from(a.b || "", "base64"));
      // 必须回 NETON 的 answer 请求本身（与 Worker /do/answer 一致返回 {ok:true}），
      // 否则 NETON 的 answer POST 挂到超时，中继循环停摆、后续隧道请求全部延迟
    ...
    if (action === "ka") {
      // 保活心跳（与 Worker /do/ka 同语义）：验证/续期许可（或现场握手）+ NETON 在线状态，
      // 不进隧道队列不耗配额。签名路径约定 "/relay/ka"，仅格式门槛
      const ip = clientIp(req);
      if (banned(ip)) return json(res, 403, { error: "temporarily banned" });
      const sign = req.headers["x-NETON-sign"] || "";
      const tsH = req.headers["x-NETON-ts"] || "";
      const nonce = req.headers["x-NETON-nonce"] || "";
      if (!SIGN_RE.test(sign) || !TS_RE.test(tsH) || !NONCE_RE.test(nonce)
        || Math.abs(Date.now() / 1000 - Number(tsH)) > 300) {
        noteBadSign(ip);
        return json(res, 403, { error: "channel signature required (NETONsign-v2)" });
      }
      const ipEnv = req.headers["x-fake-client-ip"] || ip || "";
      let permit = null;
      if (bx.bind) {
        const tok = req.headers["x-NETON-permit"] || "";
        if (tok && permitOk(bx, tok, ipEnv)) {
          permit = tok;
        } else {
          const chal = req.headers["x-NETON-challenge"] || "";
          const proof = req.headers["x-NETON-proof"] || "";
      ...
      if (permit) kaHeaders["x-NETON-permit"] = permit;
    ...
    if (action === "req") {
      // 防滥用：封禁名单 + 坏签名记账（与 Worker 站点层一致；真伪由 NETON 验）
      const ip = clientIp(req);
      if (banned(ip)) return json(res, 403, { error: "temporarily banned" });
      // 信道防护：签名三件套格式校验（与 Worker 门槛一致；真伪由 NETON 验）
      const sign = req.headers["x-NETON-sign"] || "";
      const tsH = req.headers["x-NETON-ts"] || "";
      const nonce = req.headers["x-NETON-nonce"] || "";
      if (!SIGN_RE.test(sign) || !TS_RE.test(tsH) || !NONCE_RE.test(nonce)) {
      ...
      // 否则须带一次性挑战 + 正确应答现场换许可；两者皆无 → 403，请求不进 NETON 队列
      ...
      if (envelope.stream) {
        ...
        if (permit) sHeaders["x-NETON-permit"] = permit; // 新签发/续期的许可随响应头带回
      ...
      const promise = new Promise((resolve) => {
        const timer = setTimeout(() => {
          ...
          res.end("relay: NETON offline or timeout");
          resolve(null);
        }, 15000);
...
function expire(bx, rid) {
  ...
  if (w.writer) {
    ...
    w.res.end("relay: NETON offline or timeout");
  }
}
