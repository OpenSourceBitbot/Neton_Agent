...
// 同二进制残留实例（主进程 + --NETON-guardian 守护进程）会让单实例保护令测试实例启动即退。
...
(async () => {
  ...
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "NETON-smoke-"));
  const proc = spawn(BIN, [], {
    env: {
      ...process.env,
      NETON_DATA_DIR: dir, // 隔离数据目录，不碰日常实例
      NETON_HEADLESS: "1", // 窗口隐藏不弹前台（WebView 仍正常渲染）
      NETON_NO_GUARDIAN: "1", // 测试实例不布防守护进程：结束后杀掉不会被重新拉起
    },
  ...
  try {
    ...
    await sleep(300);
    // 清理临时目录：慢 runner（如 arm）上 NETON 的 toolhomes venv 可能还有写入尾巴，
    ...
    if (ok) {
      ...
      console.error(`SMOKE FAIL: NETON exited early (code=${exited}) before ui.mounted. stderr tail:\n${errTail}`);
      process.exitCode = 1;
    } else {
