...
deb 目录下应有 NETON_<version>_<arch>.deb（arch ∈ amd64/arm64/riscv64/loongarch64/ppc64le）
输出: <out>/apt  (dists + pool 结构)   <out>/pacman  (按架构的 [NETON] 仓库)

apt 使用: deb [trusted=yes] https://yxpil.github.io/apt-repo stable main
pacman 使用: [NETON] Server = https://yxpil.github.io/pacman-repo/$arch ; SigLevel = Never
"""
import gzip
import hashlib
import io
...
PKGDESC = "NETON - 本地优先的 AI Agent 工具集（MCP / 工具注册 / 技能）"
PKGURL = "https://github.com/yxpil/NETON"
...
def gen_apt(debs, out: Path):
    ...
    for arch, items in by_arch.items():
        ...
        for deb, fields in items:
            fname = deb.name
            pool_file = f"pool/main/b/NETON/{fname}"
            ...
            lines.append(
                f"Package: {fields.get('Package', 'NETON')}\n"
...
def gen_pacman(debs, out: Path):
    """pacman 仓库: 各架构 <pkg>.pkg.tar.gz + NETON.db.tar.gz（由 deb 转换）"""
    for deb, arch, fields in debs:
        ...
        pkginfo = "\n".join(
            [f"pkgname = NETON", f"pkgver = {ver}-1", f"pkgdesc = {PKGDESC}", f"url = {PKGURL}",
             f"builddate = {builddate}", f"packager = NETON Release <yxpil@users.noreply.github.com>",
             f"size = {total_size}", f"arch = {parch}", "license = Apache-2.0", "replaces = NETON-git"]
        ...
        pkg_name = f"NETON-{ver}-1-{parch}.pkg.tar.gz"
        ...
        desc = (
            f"%FILENAME%\n{pkg_name}\n\n%NAME%\nNETON\n\n%BASE%\nNETON\n\n%VERSION%\n{ver}-1\n\n"
            f"%DESC%\n{PKGDESC}\n\n%URL%\n{PKGURL}\n\n%ARCH%\n{parch}\n\n%BUILDDATE%\n{builddate}\n\n"
            f"%PACKAGER%\nNETON Release <yxpil@users.noreply.github.com>\n\n%SIZE%\n{len(pkg_data)}\n\n"
            f"%MD5SUM%\n{hashlib.md5(pkg_data).hexdigest()}\n\n%SHA256SUM%\n{hashlib.sha256(pkg_data).hexdigest()}\n\n"
            + "".join(f"%DEPENDS%\n{d}\n\n" for d in deps)
        )
        with tarfile.open(pdir / "NETON.db.tar.gz", "w:gz") as tf:
            info = tarfile.TarInfo(f"NETON-{ver}-1/desc")
...
def gen_dnf(rpms, out: Path):
    ...
    for rpm, arch, _ in rpms:
        ...
        pkgs_xml.append(
            ...
            f"<packager>NETON Release</packager><url>{f.get(TAG_URL, PKGURL)}</url>"
            ...
            f"<rpm:vendor>{f.get(TAG_VENDOR, 'NETON')}</rpm:vendor>"
            f"<rpm:group>Applications/Development</rpm:group>"
            f"<rpm:buildhost>NETON-release</rpm:buildhost>"
