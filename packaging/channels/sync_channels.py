...
TAP = "yxpil/homebrew-NETON"
SCOOP = "yxpil/scoop-NETON"
APT = "yxpil/apt-repo"
PACMAN = "yxpil/pacman-repo"
DNF = "yxpil/dnf-repo"
DL = "https://github.com/yxpil/NETON/releases/download/{v}/{f}"
# brew/scoop 渠道必需的资产；apt 各架构缺失时跳过不阻塞
BREW_ASSETS = ["NETON_{v}_aarch64.dmg", "NETON_{v}_x64.dmg"]
SCOOP_ASSETS = ["NETON_{v}_x64-portable.zip"]
APT_ARCHES = ["amd64", "arm64", "loongarch64", "riscv64", "ppc64le"]
APT_NAMES = {"amd64": "NETON_{v}_amd64.deb", "arm64": "NETON_{v}_arm64.deb",
             "loongarch64": "NETON_{v}_loongarch64.deb", "riscv64": "NETON_{v}_riscv64.deb",
             "ppc64le": "NETON_{v}_ppc64le.deb"}
RPM_NAMES = {"x86_64": "NETON-{v}-1.x86_64.rpm", "aarch64": "NETON-{v}-1.aarch64.rpm"}
...
CASK_TPL = '''cask "NETON" do
  ...
  url "https://github.com/yxpil/NETON/releases/download/v#{{version}}/NETON_#{{version}}_#{{arch}}.dmg"
  name "NETON"
  desc "Local-first AI agent hub with MCP, tool registry and skills"
  homepage "https://github.com/yxpil/NETON"
...
def sync_brew(version, token, tmp, dry):
    ...
    put_file(TAP, "Casks/NETON.rb", CASK_TPL.format(version=v, arm=arm, intel=intel),
             token, f"NETON {v}", dry)
...
SCOOP_TPL = '''{{
    "version": "{version}",
    "description": "NETON - 本地优先的 AI Agent 工具集（MCP / 工具注册 / 技能）",
    "homepage": "https://github.com/yxpil/NETON",
    "license": "Apache-2.0",
    "architecture": {{
        "64NETON": {{
            "url": "https://github.com/yxpil/NETON/releases/download/v{version}/NETON_{version}_x64-portable.zip",
            "hash": "{hash}"
        }}
    }},
    "bin": "NETON.exe",
    ...
    "autoupdate": {{
        "architecture": {{
            "64NETON": {{
                "url": "https://github.com/yxpil/NETON/releases/download/v$version/NETON_$version_x64-portable.zip"
            }}
        }}
    }}
}}
...
def sync_scoop(version, token, tmp, dry):
    ...
    put_file(SCOOP, "bucket/NETON.json", SCOOP_TPL.format(version=v, hash=sha256_of(z)),
             token, f"NETON {v}", dry)
...
def apt_stanza(deb: Path) -> dict:
    ...
    name = deb.name
    s["Filename"] = f"pool/main/b/NETON/{name}"
...
def sync_apt(version, token, tmp, dry):
    ...
    for arch, d in debs:
        ...
        payload = {"message": f"NETON {v} {arch}", "content": b64, "branch": "main"}
        try:
            payload["sha"] = gh("GET", f"/repos/{APT}/contents/pool/main/b/NETON/{d.name}", token)["sha"]
        except RuntimeError:
            pass
        gh("PUT", f"/repos/{APT}/contents/pool/main/b/NETON/{d.name}", token, payload)
        print(f"  ✓ pool/main/b/NETON/{d.name}")
...
def sync_pacman(version, token, tmp, dry):
    ...
    if dry:
        print("  [dry] 上传各架构 pkg.tar.gz + NETON.db.tar.gz")
        return
...
def main():
    ...
    # 校验 release 存在且非预发布
    rel = gh("GET", f"/repos/yxpil/NETON/releases/tags/{a.version}", a.token)
    if rel.get("prerelease"):
        sys.exit(f"{a.version} 是预发布版本，跳过渠道同步")
    tmp = Path(tempfile.mkdtemp(prefix="NETON-channels-"))
    results = {}
